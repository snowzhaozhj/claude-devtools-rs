...
test.describe('sidebar 骨架行视觉契约（静态 opacity，无 shimmer 动画）', () => {
  ...
  test('真实骨架渲染（?forceSkeleton=1）下 .metadata-pending 子元素 computed opacity ≈ 0.55 且 animationName === none', async ({ page }) => {
    // 通过 mockIPC `?forceSkeleton=1` 钩子让 list_group_sessions 返回纯骨架，
    // sidebar 自然挂 `.metadata-pending` class——验证 svelte scoped CSS 编译后
    // 选择器在真实运行下命中（codex 三审 finding #1 提的覆盖 gap）。
    await page.goto('/?mock=1&fixture=multi-project-rich&forceSkeleton=1')
    await page.locator('.dash-row, .dash-card', { hasText: 'rust-port' }).first().click()
    await expect(page.locator('.session-item.metadata-pending').first()).toBeVisible({ timeout: 5_000 })

    const result = await page.evaluate(() => {
      const item = document.querySelector('.session-item.metadata-pending') as HTMLElement | null
      if (!item) return null
      const titleText = item.querySelector('.session-title-text') as HTMLElement | null
      const meta = item.querySelector('.session-meta') as HTMLElement | null
      if (!titleText || !meta) return null
      return {
        titleOpacity: parseFloat(getComputedStyle(titleText).opacity),
        metaOpacity: parseFloat(getComputedStyle(meta).opacity),
        titleAnimationName: getComputedStyle(titleText).animationName,
        metaAnimationName: getComputedStyle(meta).animationName,
      }
    })
    ...
    // 真实运行下 svelte scoped CSS 应让 .metadata-pending 子元素 opacity 压到 0.55
    expect(result.titleOpacity).toBeGreaterThanOrEqual(0.5)
    expect(result.titleOpacity).toBeLessThanOrEqual(0.6)
    expect(result.metaOpacity).toBeGreaterThanOrEqual(0.5)
    expect(result.metaOpacity).toBeLessThanOrEqual(0.6)

    // 关键：无 keyframe 动画（之前 shimmer 反模式）
    expect(result.titleAnimationName).toBe('none')
    expect(result.metaAnimationName).toBe('none')
  })
