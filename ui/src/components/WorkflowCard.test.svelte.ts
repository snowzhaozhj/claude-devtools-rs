...
describe('WorkflowCard 运行态（Tier 0 降级）', () => {
  test('header 显示 N agents (M done) 计数 + spinner', () => {
    const { container } = render(WorkflowCard, { props: { sessionId: "test-session", projectId: "test-project", workflow: runningTier0() } })
  ...
  test('running + 无 journal（agents 空）显示最小态而非空白', async () => {
    ...
    const { container } = render(WorkflowCard, { props: { sessionId: "test-session", projectId: "test-project", workflow: empty } })
    await expand(container)
...
describe('WorkflowCard 运行态 Tier 1（含 phases）', () => {
  test('phases 作静态列表展示在 chips 之上，agent 扁平排列（不按 phase 分组）', async () => {
    ...
    const { container } = render(WorkflowCard, { props: { sessionId: "test-session", projectId: "test-project", workflow: item } })
    await expand(container)
