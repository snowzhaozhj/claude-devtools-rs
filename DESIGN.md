...
- **计入 live signal**：任何表达"正在执行 / 正在流式 / 正在加载远端数据 / 正在 streaming"的 infinite animation —— 包括 dot ping、shimmer sweep、circular spinner、`text-shimmer`、`progress-stripe` 等。Subagent / Tool / 后台扫描的 running spinner 都计入。
