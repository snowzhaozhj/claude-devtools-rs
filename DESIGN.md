...
- System message 使用 mono pre、system surface、10px radius；与 AI message 共享同一条左侧 thread rail（3px solid `--color-border-emphasis`），通过左外侧 timeline marker 形态对立（AI 圆 / SYSTEM 方）做语义区分；header chip / bubble 起点对齐 conversation 内容基线（thread rail + 16px padding），不限 max-width 让内容右缘对齐 conversation right padding。
...
- Read/Edit/Write/Bash/Default viewer 保持同一 header、section label、copy/action button 语言，并共享**自适应分级**语义（下述 Adaptive Output Frame）——工具类型只决定内容如何渲染（代码高亮 / diff / 纯文本），不决定信息是否完整可得。
- Syntax highlighting 的 `.hljs-*` token 只能在全局 `app.css` 维护，组件内不要局部覆盖。

#### Adaptive Output Frame（自适应输出框架）

对话流与工具查看器中所有文本 / 代码 / diff 输出面的统一分级展示契约（change `adaptive-output-display`；行为真相源 `openspec/specs/session-display` + `tool-viewer-routing`）：

- **三档语义**：完整内联（短，无 chrome、无预览提示）/ 限高预览（中长，响应式限高 + 信息气味）/ 首尾切片（超大，仅行导向内容；markdown 富文本不切片，降级限高预览）。分级依据原始行数 + UTF-8 字节数（80 行/16 KiB、1000 行/256 KiB），不按渲染后 DOM 高度反推。
- **限高 token**：`--ao-preview-max-block: clamp(10rem, 30dvh, 22rem)`（`app.css :root` 单点维护）。所有限高预览 viewport 与 loading 占位一律引用该 token，禁止各 viewer 私有固定像素限高。
- **信息气味**：`{总行数} 行 · {总字节数} · 预览` 一行 mono metadata（`--color-text-secondary`），置于该输出面的共享 header（`AdaptiveOutputFrame` 自带 header 或 Read/Write/Diff 的 file header）；不用彩色 badge 或渐隐遮罩暗示截断。引用 `The Machine Information Rule` + `The Status Owns the Color Rule`。
- **两种 variant**：`code`（默认，code-bg 边框，工具输出）/ `prose`（透明底 + 细下边框，AI 文本——不为普通文本套代码块 chrome，引用 `The Tool Density Rule`）。
- **复制全文常驻**：header 内常驻低权重 icon 复制入口，复制完整原文（永不降级复制可见切片）；完整原文未就绪（懒加载中 / Missing）时禁用 + aria 原因标签。inline 档降为右上角低透明度常驻 icon。
- **内部滚动 a11y**：限高 viewport 仅在沿任一轴实际溢出时进入 Tab 序列（`adaptiveScrollViewport` attachment 统一实现：tabindex=0 + role=region + 含来源与规模的 aria-label）；竖向滚动区一律 `scrollbar-gutter: stable`；保留边界滚动链（禁 `overscroll-behavior: contain`）。
- **懒加载占位**：输出未加载时以限高档等高稳定占位渲染（`aria-busy` + "正在载入完整内容…"），加载完成不改外层 viewport 几何。

#### Output Omission Seam（省略接缝）

超大行导向输出首尾切片之间的结构性接缝：

- 中性文字（`--color-text-secondary`，11px mono）+ `1px dashed --color-border` 分隔线 + 与切片同底色；文案固定 `已省略 {N} 行 · {字节数}`（省略量 = 总量 − 首尾实渲量，精确不少算）。
- 首尾两片共享同一背景与外边界（同一输出表面的两个连续片段），不各自包卡、不加渐变 fade、不加彩色装饰。引用 `The Border Before Shadow Rule` + `The Status Owns the Color Rule`。

**The Conversation Owns the Scroll Rule.** conversation 是 SessionDetail 唯一主滚动。短输出完整内联；中长输出仅在实际溢出时形成局部可键盘进入的辅助滚动 viewport；超大行导向输出首尾切片 + 省略接缝，不新增 modal / 侧栏 / tab / drill-in 页面，不允许无限展开淹没对话流。例外：顶层主 AI 回复（lastOutput）是用户阅读正文，始终完整内联不限高（用户拍板，change `adaptive-output-display` design D10）。

**The Preview Must Declare Itself Rule.** 任何非完整可见的输出（限高预览 / 首尾切片 / 懒加载占位）必须显式声明总规模与省略 / 预览状态（信息气味 + 省略接缝），禁止用渐隐遮罩、模糊或"展开"图标暗示截断；复制动作永远针对完整原文，未就绪时禁用而非静默降级到可见片段。
