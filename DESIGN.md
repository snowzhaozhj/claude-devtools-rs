...
**The Static-vs-Live Shape Rule.** "唯一动态信号"与"识别用静态信号"必须**形态对立**，仅靠"参不参与动画"区分不够——相同颜色 + 相同 filled 形态 + halo 的多个静态点会被脉冲源"感染"，大脑把它们归入同一节律组、产生错觉脉冲。约定：动态 live 信号用 **filled dot + ring ping**；所有静态识别点用 **outline 空心圆**（透明填充 + 1.25–2px 蓝边框、无 halo）。Sidebar OngoingIndicator、SessionDetail top-stat LIVE dot、ai-thread-node-live、SubagentCard running 标记一律 outline；唯一 filled dot 在 OngoingBanner（详情页 primary live 信号）。

**The One Live Signal Rule.** 单个 surface（Sidebar / SessionDetail / Dashboard）同时显示的**动态**（CSS `animation: ... infinite` 持续运行的）live 指示器最多一个；其余 live 指示器必须退为静态 outline ring（按 `Static-vs-Live Shape Rule`）。SessionDetail 的 primary live 信号是 `OngoingBanner` 的 filled dot ping；其它位置（top stat LIVE dot、ai-thread-node-live、SubagentCard running 标记）使用 outline 形态保持蓝色识别但不参与脉冲。Sidebar 因为 N 个 ongoing 会同时存在，N 个并发脉冲会形成视觉噪音，因此 Sidebar OngoingIndicator 是**永远静态**的。
