...
| `crates/cdt-discover/src/agent_configs.rs` | `~/.claude/agents/*.md` + 本地 `ProjectScanner` 列出的 project cwd 下 `.claude/agents/*.md`——永远本机 Local（IPC handler 调 `self.scanner` 即 Local scanner，SSH project 不走此路径，前端 subagent 颜色 badge 仅查本机 agent 配置），与 design D7 同型 Local-only 业务（与 `cdt-config/**` / `cdt-discover/src/wsl.rs` 同 justification） |
| `crates/cdt-api/src/ipc/local.rs` | `list_jobs_from_dir` 读 `~/.claude/jobs/` 永远 Local-only（后台 job state.json 只存本机），不参与 SSH context，与 design D7 同型 Local-only 业务 |
