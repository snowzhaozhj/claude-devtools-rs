...
| 文件 | 覆盖 |
|---|---|
| `opsx-apply-cadence.md` | PR 推进流水线节拍（业务段 + 发布尾段 N.1-N.4）|
| `codex-usage.md` | codex 二审 / rescue 何时调 + prompt 模板 |
| `perf.md` | 性能预算 / 反模式 / bench 入口 / PR 模板 |
| `parallelism-modes.md` | 三种并行执行形态（subagent / agent team / bg）的分派 |
...
本仓 Rust 重写原 TS 项目的根本动机就是性能。**预算 / 反模式 / bench 入口 / PR 模板**沉淀在 `.claude/rules/perf.md`——任何涉及启动路径 / IPC / 后端算法 / 列表渲染的 PR 都 SHALL 按那里的规则评估 perf impact。
...
- **本仓自建 skill**（`.claude/skills/<skill>/SKILL.md`）：
  ...
  - 性能：`perf`（统一入口含 SessionDetail bench 跑分 + idle CPU 诊断两个子模式 + 共用 bundled script）
...
- **Subagent**：`spec-fidelity-reviewer` 按 capability 审计 scenario→test 覆盖；`rust-conventions-reviewer` / `tauri-config-reviewer` / `windows-compat-reviewer` 按域审查；`codex:codex-rescue` 跑异构二审；`pr-review-toolkit:silent-failure-hunter` / `pr-test-analyzer` / `code-simplifier` 按条件在 N.3 并行调（详 `.claude/rules/codex-usage.md::与 pr-review-toolkit 互补`）。
...
5. **默认每个 PR push 后**都调 codex 异构二审 + 按条件调 pr-review-toolkit 专项审查（`silent-failure-hunter` / `pr-test-analyzer`），三轨并行。豁免规则见 `.claude/rules/codex-usage.md`。**codex CR 可能多轮**：每轮都用 `SendMessage` 接续同一 agentId 复用 context，不要起新 agent 重头读。
