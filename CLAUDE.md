...
7. **多任务并行**：N≥2 个独立可并行 PR SHALL 用 `claude --bg` 不用 subagent（subagent 跑长流水线会卡主 session 或 context 爆）。**拆分前用判断框架**（独立 / 可验证 / 工作量值得 / wall time 价值，4 个 ✓ 才拆，否则合并 1 PR）。详见 `.claude/rules/bg-task-dispatch.md`、prompt 模板 `.claude/templates/bg-pr-pipeline.md`、一键起 `just bg-pr <name> <prompt>`。
