...
- **插件**：`pr-review-toolkit`（PR 二审，与 codex 并行）；`impeccable`（UI 视觉契约）。
...
5. **默认每个 PR push 后**都并行调 codex 二审 + `pr-review-toolkit:review-pr`。豁免：bump version / 纯 docs / typo / CI 配置微调。**codex CR 可能多轮**：每轮都用 `SendMessage` 接续同一 agentId 复用 context。
