...
| `.claude/rules/bg-task-dispatch.md` | 主 session 之外起并行执行单元前（覆盖三形态：subagent / agent team / bg） |
...
大会话首屏卡顿走 IPC payload 瘦身路径 —— 模式见 `src-tauri/CLAUDE.md::IPC payload 瘦身模式`。历次实现查 `git log --grep="feat(perf)"`。
...
7. **并行执行形态选择**（详见 `.claude/rules/bg-task-dispatch.md`「形态选择决策树」）：单点 PR 主 session 自跑 + subagent 按需调；中等 PR 主 session + 多 subagent 并行 review；**大改动**（>2 天多角色协作 / 视觉重构 / 跨 capability）SHALL 用 **Agent team**（lead + teammate Mailbox 互通，需 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`）；**N≥2 独立完整 PR** SHALL 用 `claude --bg` 不用 subagent（subagent 跑长流水线会卡主 session 或 context 爆）。拆 PR 前用 4 个 ✓ 框架（独立 / 可验证 / 工作量值得 / wall time 价值），任一不满足合并 1 PR；一键起 `just bg-pr <name> <prompt>`。
