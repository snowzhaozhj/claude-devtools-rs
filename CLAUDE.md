...
7. **并行执行形态选择**（详见 `.claude/rules/parallelism-modes.md`「形态选择决策树」）：单点 PR 主 session 自跑 + subagent 按需调；中等 PR 主 session + 多 subagent 并行 review；**大改动判定** `> 2 天工作量 AND (多角色协作 OR 视觉重构 OR 跨 capability)` 任一特征命中 → SHALL 用 **Agent team**（lead + teammate Mailbox 互通，需 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`）；**N≥2 独立完整 PR** SHALL 用 `claude --bg` 不用 subagent（subagent 跑整条流水线会卡主 session 或 context 爆；codex-rescue 二审本身是 subagent 用法不在此禁令）。拆 PR 前用 4 个 ✓ 框架（独立 / 可验证 / 工作量值得 / wall time 价值），任一不满足合并 1 PR；一键起 `just bg-pr <name> <prompt>`。
