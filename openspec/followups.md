...
### [coverage-gap] SSH context 下 get_image_asset / get_tool_output 不走 LRU cache

change `fix-ssh-active-context-dispatch` 的 SSH 分支用 `fs.read_to_string(path) + parse_jsonl_content + Arc::new` 直接构造 `Arc<Vec<ParsedMessage>>`，**不**走 `ParsedMessageCache`——`cache_signature::FileSignature::from_metadata(&std::fs::Metadata)` 接 local metadata，SSH 路径用的 `FsMetadata` 不兼容。同一 session 多次调 image / tool output 会重复 SFTP 读 + parse。修法：把 `FileSignature` 加 `from_fs_metadata(&cdt_discover::FsMetadata)` constructor，cache key 改成 `(context_id, path)` 元组避免本地/远端同 path 串。
