...
### [perf] 首次点开大会话明显延迟
- **用户报告**（2026-04-19）：一个 976 条消息的会话，首次点开时首屏渲染有明显延迟；切回同 session 时不慢（说明瓶颈在首次数据加载或首渲染链路，不是每次渲染）。
- **未调查**。怀疑方向（**下次复现时按顺序排查**）：
  1. IPC `get_session_detail` 同步扫整个 JSONL + `build_chunks_with_subagents` 的耗时（cdt-parse 解 976 行 + cdt-analyze 建 chunk / 配对 tool + resolve subagents）
  2. SessionDetail 首次挂载时 `marked` + `highlight.js` 语言按需加载 + `mermaid` 动态 import 的串行开销
  3. Chunk 列表无虚拟滚动，N 条 AIChunk/UserChunk 全量挂载 DOM（对比 Sidebar 的虚拟滚动已接 `@tanstack/virtual`）
  4. `buildDisplayItemsFromChunks` 对每个 subagent.messages 递归建 DisplayItem，深度嵌套时 O(N·D)
- **复现条件**：打开任一 ≥500 条消息的 session，观察点击 Sidebar 到首屏渲染完成的耗时（可用 Chrome DevTools Performance 录 1 轮）。
- **下次开工 checklist**：
  - [ ] 加一条 `tracing::info!` 在 `LocalDataApi::get_session_detail` 前后打时间戳（后端 IPC 贡献多少）
  - [ ] 前端 `SessionDetail.svelte` `$effect` 首次解析 detail 前后 `performance.now()` diff（前端解析 + 渲染贡献多少）
  - [ ] 根据 2:8 分布决定优化方向：后端占大头 → 同步扫优化（流式 parse / 分片渲染）；前端占大头 → 虚拟滚动 / 异步加载 markdown engine / 懒渲染 tool viewer
- **不做**：立即加虚拟滚动。根因未定位前不引入新依赖。
