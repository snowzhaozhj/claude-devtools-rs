...
- **后续**：
  ...
  - **下一轮 follow-up**（如仍有感知）：剩余瓶颈在 46a25772 类无 image 的 ai-heavy 场景：`response_content`（16%）+ ToolResult `other`（11%）；可考虑 ToolExecution.input/output 与 ToolResult content 也按需懒拉（更细粒度），但收益小（< 30%），改动远大于本次。
