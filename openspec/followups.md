...
### [coverage-gap] file-change 自动刷新后 SearchBar mark 索引过期 ⚠️ codex review 2026-05-03 找到，未修
...
- **影响**：file-change 期间用户用搜索的体验劣化——但实际窄场景（要 query 处于 visible + 同时 file-change 写入新内容）。`search-hydrate-lazy-chunks` 修了"未渲染段命中"主路径，本条是次要交互 bug，可容忍。
- **修复路径**：SearchBar 加 `contentVersion: number` prop，SessionDetail 在 `detail = d` 后递增；SearchBar 内 `$effect(() => { if (visible && query && contentVersion) doSearch(); })` 自动重搜。
- **不做**：先观察是否真有用户在 file-change 期间用搜索。修法成本低但要补 design 决策 + spec scenario + 单测，独立 change 走更干净。
