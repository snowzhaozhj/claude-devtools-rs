...
### [coverage-gap] `file-change` 事件未桥到前端，打开的会话不会自动刷新
...
- **Rust port 现状**：
  - `FileWatcher` 正常跑，`FileChangeEvent` 被 `NotificationPipeline` 消费用于错误通知
  - `src-tauri/src/lib.rs` **没有** emit `file-change` 给前端
  - `ui/src/routes/SessionDetail.svelte:54-75` `onMount` 一次性 `getSessionDetail`，无 listener，打开会话后不会自动追加新消息
- **实现路径**：
  1. 后端 `src-tauri/src/lib.rs` setup 里再 `api.subscribe_files()` 一份（或在已有 notifier bridge 旁新增），`tauri::async_runtime::spawn` 转发 `FileChangeEvent` 到前端 `file-change` 事件
  2. 前端新增 `ui/src/lib/fileChangeStore.svelte.ts`（或 App.svelte 直接 listen），分发到：
     - 当前打开的 session tab → 调 `getSessionDetail` 替换 `tabStore` 缓存 + UI 刷新（保留滚动位置：事件中判断若 `conversationEl.scrollTop + clientHeight >= scrollHeight - N` 视为 pinned to bottom，刷新后保持贴底）
     - 当前 project 的 sidebar → 调 `list_sessions` 刷新列表（命中新 session id 时）
  3. 防抖：参考原版 `sessionDetailSlice` 的 in-flight dedupe，同一 session 的并发刷新要合并（可以用 `Map<sessionId, Promise>`）
- **openspec 影响**：`session-display` + `sidebar-navigation` 两个 UI spec 各加一条 SHALL（"文件变更时 UI 自动刷新"）；可能需要 `ipc-data-api` 暴露 file-change event channel 的 SHALL

### [coverage-gap] Session "in progress" 状态检测与 ongoing/interruption UI 未实现
- **原版行为**（`src/main/utils/sessionStateDetection.ts::checkMessagesOngoing`）：
  - 按顺序记录活动栈：`thinking` / `tool_use` / `tool_result` / `text_output` / `interruption` / `exit_plan_mode`
  - 找最后一个 ending 事件（`text_output` / `interruption` / `exit_plan_mode`），若之后还有 AI 活动（`thinking` / `tool_use` / `tool_result`）→ 判定 ongoing
  - UI：Sidebar 绿点脉冲（`SessionItem.tsx`） + 会话底部 "Session is in progress..." 蓝色横幅（`OngoingIndicator.tsx::OngoingBanner`） + AIGroup 最后一项换 spinner
- **中断识别信号**（三路）：
  1. user 消息 text 以 `[Request interrupted by user` 开头（用户按 Esc）
  2. user 消息 `toolUseResult === 'User rejected tool use'`（工具拒绝）
  3. assistant `SendMessage(type=shutdown_response, approve=true)`（agent 主动退出）
- **Rust port 现状**：未实现。
  - `cdt-analyze` 无 `check_messages_ongoing` 等价函数
  - `SessionSummary` / `SessionDetail` 无 `isOngoing` 字段
  - UI 无 `OngoingIndicator.svelte`
- **[impl-bug?] interrupt marker 被错误列入 hard noise**：
  - `crates/cdt-parse/src/noise.rs:13` 的 `INTERRUPT_PREFIX = "[Request interrupted by user"` 把中断消息归为 **hard noise 完全过滤**
  - 与原版"当 ending event 保留并作为 `interruption` semantic step 显示"相反
  - 修前必须先决定：是把 interrupt 从 `HardNoise` 拎出来成独立 category（推荐），还是在 `chunk-building` 层恢复
- **实现路径**：
  1. `cdt-parse`：`MessageCategory` 新增 `InterruptMarker` 变体，从 `HardNoise` 拆出；`noise.rs::INTERRUPT_PREFIX` 识别路径不变但不再归 noise
  2. `cdt-analyze`：新增 `semantic_step::Interruption { text, timestamp }`；`check_messages_ongoing(messages: &[ParsedMessage]) -> bool` 端口 TS 算法；`build_chunks` 把 `InterruptMarker` → `Interruption` step
  3. `cdt-api`：`SessionSummary` / `SessionDetail` 加 `is_ongoing: bool`；`session_metadata.rs` 轻量扫描阶段也计算 ongoing（可选优化：仅看尾部 N 条）
  4. UI：`OngoingIndicator.svelte`（两变体：sidebar dot / chat banner）；`Sidebar.svelte` 根据 `isOngoing` 显示绿点；`SessionDetail.svelte` 底部根据 `isOngoing` 显示横幅；interruption step 单独渲染（类似"已被用户中断"块）
  5. 依赖：ongoing 需要配合 **上一条**（实时刷新）才完整 —— 没有实时刷新时 `isOngoing` 在用户切 tab 前不会变化
- **openspec 影响**：
  - `chunk-building` spec delta：interrupt marker 从 HardNoise 拎出来（同时反转 followups 里"已在 session-parsing spec 标 hard noise"那条）
  - 新增/扩展 `session-display` spec：ongoing 判定算法 + UI 表现（绿点 / 横幅 / interruption step）
  - `ipc-data-api` spec delta：`SessionSummary` / `SessionDetail` 增加 `isOngoing` 字段
  - 建议 change 名：`port-session-ongoing-and-interruption`

### 建议实现顺序
1. **先做实时 `file-change` 桥**（独立价值 + 后续基础）
2. **再做 ongoing + interruption**（依赖 #1 才能"看到绿点变白"）
