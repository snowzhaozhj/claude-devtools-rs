...
The system SHALL validate IPC input parameters and return structured errors (error code, human message) instead of throwing on invalid input.

#### Scenario: Missing required field
- **WHEN** a caller invokes an operation missing a required field
- **THEN** the response SHALL contain a validation error describing the missing field
