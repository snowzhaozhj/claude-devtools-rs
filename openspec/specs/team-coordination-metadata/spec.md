...
系统 SHALL 提供 `parse_all_teammate_attrs(msg) -> Vec<TeammateAttrs>`，使用全局 regex `<teammate-message\s+teammate_id="([^"]+)"([^>]*)>([\s\S]*?)</teammate-message>` 抽取**每个**独立 teammate-message 块。一条 user 消息含 N 个块时返回 N 条独立 `TeammateAttrs`。

`parse_teammate_attrs(msg) -> Option<TeammateAttrs>` 保留为兼容入口，调用 `parse_all_teammate_attrs` 后返回首条；新代码 SHALL 优先用多 block 入口。

chunk-building 在 teammate user 分支 MUST 调 `build_pending_teammates(msg)`（基于 `parse_all_teammate_attrs`），把每个块都转成独立 `TeammateMessage` push 到 `pending_teammates`。多 block 时各条 uuid SHALL 加 `-{idx}` 后缀，避免下游 `{#each}` key 冲突；timestamp 共享原 user 消息时间戳。
...
- **THEN** `parse_all_teammate_attrs` SHALL 返回 3 条 `TeammateAttrs`，分别为 `{teammate_id: "alice", body: "A"}`、`{teammate_id: "bob", body: "B"}`、`{teammate_id: "charlie", body: "C"}`
