...
合并后同一 `(scope, name)` 出现重复时 SHALL 沿用既有的"按 `(scope_global_first, name)` 排序去重"策略，去重保留 IPC 入口收到的 pairs 中先出现的条目（实现上 IPC 入口 SHALL 把多 cwd 的 pairs 按 session mtime 倒序展开，使最新 cwd 的 agent 优先）。

调用方 IPC 路径 `LocalDataApi::read_agent_configs` SHALL 在构造 `Vec<(project_id, cwd)>` pairs 时，对每个 `Project` 收集其 sessions 列表里所有非空 `cwd` 值的去重集合，**按 session mtime 倒序**笛卡尔展开为 pairs；当 session 无 `cwd` 时 fallback 到 `Project.path`。`cdt_discover::agent_configs::read_agent_configs(pairs)` 公开签名不变。
