...
2. allowlist：provider 实现文件（`crates/cdt-fs/src/local.rs` 等）、`crates/cdt-discover/src/{wsl,worktree_grouper}.rs`、`crates/cdt-cli/**`、`crates/cdt-watch/**`、`crates/*/tests/**`、`crates/*/examples/**`、`crates/*/benches/**`
3. 匹配模式：`tokio::fs::metadata` / `tokio::fs::read` / `tokio::fs::File::open` / `tokio::fs::read_to_string` / `tokio::fs::read_dir` 等
4. 退出码：non-allowlist 命中时 exit 1（CI 拒），但**本 change 期间** SHALL 允许通过 `--warn-only` flag 切换为 warning（exit 0）
...
#### Scenario: 本 change 期间 warning-only

- **WHEN** 本 change 合并后第一次跑 CI xtask check
- **THEN** xtask SHALL 以 `--warn-only` 模式运行，把现有 ~34 处违反列为 warning，CI 不 fail
- **AND** 未来 PR-D 完成后另开 PR 把 `--warn-only` 去掉切 fail-on-match
