...
这意味着 Claude Read 工具的 `output.text` 完整保留原始 cat -n 风格 `<num>\t<text>` 行号前缀；UI 渲染层（`session-display` capability 的 `ReadToolViewer`）按需 strip 前缀，避免与 CSS `::before data-line` 双重显示行号。
