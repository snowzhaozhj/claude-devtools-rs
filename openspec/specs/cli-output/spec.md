...
`cdt session <id> --chunks` 命令 SHALL 支持 `--content <omit|overview|full>` flag，控制 JSON/JSONL 输出中 chunk 内容的详细程度。
...
指定 `--content overview` 时，输出 SHALL 为每 chunk 一条结构摘要：
- `chunkIndex`、`kind`、`timestamp`、`toolNames`（数组）、`errorCount`、`headline`（首 100 字符）
...
- **WHEN** 运行 `cdt session <id> --chunks --format json` 不带 `--content` flag
- **THEN** 输出 SHALL 与当前行为一致（raw `SessionDetail` JSON）

#### Scenario: --content omit 输出 chunk 结构概览

- **WHEN** 运行 `cdt session <id> --chunks --format json --content omit`
...
#### Scenario: --content overview 输出每 chunk 一行摘要

- **WHEN** 运行 `cdt session <id> --chunks --format json --content overview`
- **THEN** 每个 chunk SHALL 包含 `chunkIndex`、`kind`、`timestamp`、`toolNames`、`errorCount`、`headline`

#### Scenario: --content full 输出完整内容

- **WHEN** 运行 `cdt session <id> --chunks --format json --content full`
- **THEN** 每个 tool execution SHALL 包含完整 `input` 和 `output`
- **THEN** 每个 response SHALL 包含完整 `content`

#### Scenario: --content omit 与 grep 的交互

- **WHEN** 运行 `cdt session <id> --chunks --content omit --grep <keyword>`
...
- **WHEN** 运行 `cdt session <id> --chunks --format table --content omit`
- **THEN** table 输出 SHALL 与不指定 `--content` 时一致

#### Scenario: --content 非法值报错

- **WHEN** 运行 `cdt session <id> --chunks --content invalid`
- **THEN** SHALL 报错，提示合法值为 `omit`、`overview` 或 `full`
...
`cdt session <id> --chunks` 的 `--range` 和 `--tail` SHALL 互斥，同时指定 SHALL 报错。

#### Scenario: 同时指定 range 和 tail 报错

- **WHEN** 运行 `cdt session <id> --chunks --range 0:10 --tail 5`
