...
会话列表页在桌面 IPC 与 server mode HTTP 两条链路上的加载行为完全分叉：IPC 走骨架快返 + 后台 metadata broadcast emit（~5–20 ms 首帧），HTTP 走 `list_sessions_sync` 完整同步扫描（100–500 ms 阻塞 fetch），导致 server mode 翻页明显卡顿；桌面端虽快但骨架字段（`title=null` / `messageCount=0` / `isOngoing=false`）到真值之间的"字段跳现"、切 project 时的 race window、以及**冷启动时 `MetadataCache` 全 miss 导致所有 session 都要等后台扫描 emit 才能填上真值**——三类视觉断层都带来可感知的闪烁。两条路径的 spec 前提（`ipc-data-api` §"HTTP 保留同步完整返回"理由是"HTTP 无 push 通道"）已被 `add-server-mode` change 引入的 `/api/events` SSE bridge + `session_metadata_update` 推送通道推翻，统一两条链路既能消除卡顿、又能让两端体验一致，并去掉一个长期的协议分叉。
...
- **MODIFIED** `ipc-data-api`（追加冷启动持久化）：`MetadataCache` SHALL 在进程退出前 dump 到 `app_data_dir/session-metadata-cache.json`、在进程启动时 hydrate；hydrate 仅恢复条目本体，**`FileSignature` 等价校验仍是真相源**（mtime / size / identity 任一不等时仍按 miss 处理，自动 fallback 到原后台扫描路径）。冷启动 + 文件未变化的场景下骨架阶段 SHALL 直接命中 cache、带真值返回（zero emit），消除"字段跳变"的冷启视觉断层。
...
- **代码**：
  ...
  - `crates/cdt-api/src/ipc/parsed_message_cache.rs`：`MetadataCache` 加 `dump_to_disk(path)` / `load_from_disk(path)`；序列化格式 `serde_json` + 顶层 schema version 字段（不等版本 SHALL 静默跳过 hydrate）
  - `src-tauri/src/lib.rs`：Tauri `setup` hydrate cache、`on_window_event` Destroyed 时 dump；`cdt-cli` 同样 hook 启动 / 退出（保持桌面 / server / cli 三态对齐）
  - `ui/src/lib/transport.ts::BrowserTransport`：首次 `invoke('list_sessions', ...)` 前 SHALL await SSE OPEN（新建 `ensureSseReady()` 内部方法）
  - 新增 `ui/src/lib/sessionListStore.svelte.ts`：by-projectId LRU cache + SWR + generation token
...
- **测试**：
  ...
  - `crates/cdt-api/tests/`: 新增 `metadata_cache_persistence.rs` 集成测试覆盖 dump/hydrate 往返 + `FileSignature` 变更时 miss + schema 版本不等静默跳过
  - `ui/src/lib/sessionListStore.test.ts`: store unit test（SWR / cancel / debounce）
  - `ui/tests/e2e/`: 新增 Playwright spec 覆盖快速翻页不卡顿 + 切 project 不闪烁 + 冷启动（清磁盘 cache）首帧字段连贯
- **依赖**：无新增（AbortController / EventSource 均为浏览器原生，generation token / LRU 自实现）
- **性能**：server mode 翻页 100–500 ms → ~10–20 ms（与 IPC 同档）；冷启动场景 `list_sessions` emit 次数从"每条 miss 都 emit" → "FileSignature 等价时零 emit"，骨架阶段直接命中真值；两条链路 user/real / RSS 预算不变；新增前端 store 内存预算：< 1 MB（默认 LRU capacity 16 个 project × 平均 200 条骨架 × ~120 byte ≈ 380 KB）；新增持久化 cache 磁盘占用预算：< 2 MB（capacity 200 × ~10 KB metadata snapshot ≈ 2 MB 上限）
