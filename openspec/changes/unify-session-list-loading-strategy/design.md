...
### D7 — `MetadataCache` 持久化（消除冷启动闪烁）

**决策**：`cdt-api::ipc::parsed_message_cache::MetadataCache` 新增：

```rust
#[derive(serde::Serialize, serde::Deserialize)]
struct CacheSnapshot {
    schema_version: u32, // 当前 1；不匹配静默跳过
    entries: Vec<(FileSignature, SessionMetadata)>,
}

impl MetadataCache {
    pub fn dump_to_disk(&self, path: &Path) -> Result<(), CacheError>;
    pub fn load_from_disk(path: &Path) -> Result<Self, CacheError>;
}
```

- **dump 时机**：`src-tauri/src/lib.rs` `on_window_event` `CloseRequested` / `Destroyed`、`cdt-cli` SIGTERM/SIGINT handler；同步写避免 tokio 关闭竞争。**异步 dump + 退出前 join 5 s 超时**（不是同步阻塞写）——`SessionMetadata.title` 含 UTF-8 中文 / `gitBranch` 长 ref name 时单 entry 可超 50 KB，capacity=200 全量 dump 实测最大可达 10 MB，HDD 上同步 fsync 200–500 ms 卡退出；改异步写入 + 5 s join 超时避免卡退出窗口，超时则 `tracing::warn!` 放行（下次启动 cache 空，与未持久化退化等价）。
- **hydrate 时机**：Tauri `setup` 创建 `LocalDataApi` 之前 + `cdt-cli` `main` 启动时（路径取 `cdt_discover::app_data_dir().join("session-metadata-cache-<process>.json")`，详见下文 process 区分）。
- **校验**：hydrate 仅恢复 cache map；`try_lookup_cached_metadata` 仍按 `FileSignature` 等价校验（mtime/size/identity 任一不等则 miss），所以"持久化条目过期"自动 fallback 到后台扫描，不引入新校验逻辑。`FileSignature.identity`（Unix `(dev, ino)`）随外置磁盘 remount 改变 → hydrate 命中率下降是 acceptable trade-off（罕见场景；命中率下降的代价仅"冷启字段跳变恢复一帧"）。
- **schema version**：未来 `SessionMetadata` 加字段时 bump version，旧版本 dump 直接跳过 hydrate（不破坏功能，只是冷启不命中）。
- **同 process 内 IPC + server-mode HTTP 共享 cache**：桌面 app 在 `src-tauri/src/lib.rs` 中**通过 `Arc<LocalDataApi>` 共用同一个 `LocalDataApi` 实例**（IPC handler 与 axum `AppState.api` 持同一 Arc），所以 cache 是同一 `Arc<Mutex<MetadataCache>>`，dump/hydrate 也只发生一次——**不**存在同 process 内多实例并发 dump 互覆盖的问题。
- **跨 process（Tauri 桌面 app + 独立 `cdt-cli` 同时跑）的并发写**：两个 process 各自持有独立 cache，若都 dump 到同一路径会 last-write-wins 静默丢失另一 process 的 entry。**修法：按 process 类型分文件路径**——Tauri 桌面 app 用 `session-metadata-cache-tauri.json`，`cdt-cli` 用 `session-metadata-cache-cli.json`。两种 process 仍可分别 hydrate 自己的快照；不引入 advisory lock（跨平台 `flock` 语义复杂、Windows 无原生支持）。

**Alt 1**（rejected）：前端 sessions store 也持久化到磁盘。

理由：前端持久化引入"显示的 session 后端已不存在"的 ghost 问题（reconcile 复杂），后端 cache 持久化 + `FileSignature` 真相源已经能让前端冷启 SWR 命中真值，无需双层。

**Alt 2**（rejected）：把 `MetadataCache` 后端从内存 LRU 升级为 `sled` / `rocksdb` 嵌入式 KV。

理由：依赖体积膨胀、引入异步 fsync 时机问题；JSON dump 简单可逆，capacity=200 全量 dump < 5 ms。
...
- **R4：`MetadataCache` 持久化 dump 失败磁盘满 / 权限不足** → 静默忽略（`tracing::warn!` 记录），下次启动 hydrate 失败 → cache 空，自动 fallback 到后台扫描；功能无回归。
- **R5：`MetadataCache` 持久化的安全/隐私** → snapshot 含 `title` / `gitBranch` 等用户内容，存 `app_data_dir`（OS 用户目录权限隔离），与 JSONL 自身权限同档；不引入新泄露面。
...
1. **Phase A（后端独立可发）**
   ...
   - `cdt-api/src/ipc/parsed_message_cache.rs` 加 dump/hydrate
   - `src-tauri/src/lib.rs` setup hydrate + on_window_event dump
   - `cdt-cli` 启动 hydrate + signal handler dump
   - 测试：`crates/cdt-api/tests/http_list_sessions_skeleton_then_sse.rs` + `metadata_cache_persistence.rs`
...
**Rollback**：`cdt-api/src/http/routes.rs::list_sessions` 改回 `list_sessions_sync` 即恢复 server mode 旧行为；前端 store / 持久化 cache / 视觉渐显独立，不引入新 spec 契约的话也可逐项 revert，互不依赖。

## Open Questions

- **Q1**：`MetadataCache` capacity 是否随持久化提高？当前 200 是内存级合理值，持久化后磁盘 ~2 MB 上限可拉到 500，但更高的 hit rate 收益对应到长尾使用。**默认保持 200**，留 follow-up 观察实际 cache miss 率再调。
...
- **Q3**：`MetadataCache` dump 在 `CloseRequested` 与 `Destroyed` 两个事件谁先触发？Tauri docs 不明确，**采用 `CloseRequested`**（更早、用户主动关闭时一定触发；Destroyed 在某些平台仅 SIGKILL 才触发）。
