...
- handler SHALL **消费 response** 通过 `mergeRecoveryResponse(sessions, result.items)` 与 `applyPendingMetadata` 写回 sessions + store。`mergeRecoveryResponse` 是 SSE 恢复路径**专用**合并（**不**复用 `mergeSilentMetadata` / `mergeSessions`）：
...
- 设计取舍（codex 二审 4 轮验证后定型）：
  ...
  - **round 5（当前定型）**：用专用 `mergeRecoveryResponse`——response 真值优先覆盖 prev stale 真值；response 骨架则保留 prev（cache miss 经 SSE patch 写真值），0 新 IPC、0 后端改动、0 contract 破坏
...
- **AND** Sidebar SHALL **消费** response 通过 `mergeRecoveryResponse(_, result.items)` + `applyPendingMetadata` 写回 sessions 与 store 缓存——cache hit 项的真值从 response 拿（让 response 真值覆盖 prev stale 真值）
