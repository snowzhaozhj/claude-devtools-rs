...
- handler SHALL **消费 response** 通过 `mergeSessions(sessions, result.items, true)` 与 `applyPendingMetadata` 写回 sessions + store——**cache hit 真值仅在 response 里**（后端 `try_lookup_cached_metadata` fast-path inline 返完整 metadata，**不**入后台扫描 spawn、**不** emit SSE patch）。**cache miss 真值**仍走 SSE patch 路径——后端 spawn 后台扫描后广播 `SessionMetadataUpdate`，UI 既有 `session-metadata-update` listener 写回；response 里 cache miss 项是骨架，`mergeSilentMetadata` 保证不覆盖 prev 已 patch 真值
...
- 设计取舍（codex 二审 4 轮验证后定型）：
  ...
  - **round 4（当前定型）**：listSessions(已加载范围) + **消费 response** 走 mergeSessions/applyPendingMetadata 写回——cache hit 真值从 response 拿、cache miss 真值从 SSE patch 拿，0 新 IPC、0 后端改动、0 contract 破坏
...
- **AND** Sidebar SHALL **消费** response：cache hit 项的真值从 response 拿，通过 `mergeSessions(_, result.items, true)` + `applyPendingMetadata` 写回 sessions 与 store 缓存
- **AND** cache miss 项的真值仍走后端 spawn 扫描 + SSE 广播 `SessionMetadataUpdate` 的 listener patch 路径写回
