...
- 两个 event 共用同一恢复 handler：当前 `selectedProjectId` 非空时调 `listSessions(projectId, Math.max(sessions.length, SESSION_PAGE_SIZE))` 触发后端按**已加载范围**重新 spawn metadata 扫描——后端 `LocalDataApi::list_sessions` 按 `take(pagination.page_size)` 截断，pageSize=sessions.length 等价于"扫描已加载范围"，spawn 的后台 task 对每条 cache miss 都会通过 SSE 广播 `SessionMetadataUpdate`；UI 走既有 `session-metadata-update` listener patch 路径正常写回 sessions + store 缓存
- handler **不**消费 listSessions response（response 是骨架且 SSE 已恢复，patch 才是真值通道）；fire-and-forget catch 仅记录 warn 不打扰 UX
- 设计取舍（codex 二审 round 3 验证后定型）：
  - 仅重扫 page 1（silent loadSessions）→ page 2+ pending 永久卡空（round 2 反例）
  - 调 `getSessionSummariesByIds` 补齐 → 该 IPC 返 light skeleton（`title=null` / `messageCount=0`）契约固化（`crates/cdt-api/tests/ipc_contract.rs::get_session_summaries_by_ids_returns_light_summaries`），骨架对 batch fetch 也无效（round 3 反例）
  - **当前方案**：直接调 listSessions IPC + 已加载范围 pageSize → 复用 `list_sessions` 既有"骨架 + spawn + SSE patch"骨架，0 新 IPC、0 后端改动、0 contract 破坏
...
- **THEN** Sidebar SHALL 调 `listSessions(projectId, 60)` 触发后端按已加载 60 条范围 spawn metadata 扫描
- **AND** 后端 spawn 的扫描对每条 cache miss SHALL 通过 SSE 广播 `SessionMetadataUpdate`，UI listener patch 写回 sessions + store
- **AND** 不消费 listSessions response 本身（骨架不带真值）；fire-and-forget
