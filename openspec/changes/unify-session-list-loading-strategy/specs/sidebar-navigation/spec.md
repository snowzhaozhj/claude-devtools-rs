...
- 两个 event 共用同一 silent refresh handler：当前 `selectedProjectId` 非空时调 `loadSessions(selectedProjectId, true)` 走 silent merge 路径；后端在 silent reload 触发的新一轮 `list_sessions` 调用中重启扫描 + 重新 emit `SessionMetadataUpdate` patch（此时 SSE 已 OPEN / Lagged 之后 stream 已恢复），listener 将真值写回 sessions 与 store 缓存
- silent merge SHALL 用 `mergeSilentMetadata` 保留已 patch 真值（**不**用骨架 `null` 覆盖）——即使本次重启扫描的部分 patch 仍未到达，UI 也保留之前已经看到的真值；新到达 patch 通过 `session-metadata-update` listener 正常写入
