...
- `sidebar-navigation` 44 hits 降至约 11（保留约 11 处用户感知阈值数字作 NFR 契约——按 D-2c 三分；分布：1 处 600ms toast + 10 处 100/150/200/250/1500 ms 渐显与 debounce 阈值）
...
- 不动 Purpose 段（已经简洁）
- 不重命名 Requirement 标题中的 `Store loadMore 实现 leading + trailing debounce 100 ms` 这类内部 store API 名（标题级 RENAMED 工艺成本相对收益不划算，留作后续 spec 重组 PR 一起处理）
...
| 100 ms（store loadMore leading+trailing debounce 阈值）| Requirement body + Scenario | **用户感知**（"100 ms 是人类滚动停顿阈值"，body 已自述）| **保留** —— 这是用户能区分"连续滚动 vs 停顿"的阈值，即 PRODUCT.md 类用户感知契约。仅删 body 中"`lastFiredAt` 距 now ≥ 100 ms" / "重置 trailing timer 到 `lastFiredAt + 100 ms`" 这种内部 timer 实现细节句，把 100 ms 留在"用户感知阈值"叙述句 + Scenario WHEN/THEN |
...
**结果**：18 hits → 11 hits（保留的 11 处都是 Scenario WHEN/THEN 内或 body 中可断言用户感知契约：600ms toast / 100 ms debounce 阈值（多次出现于 body 阈值叙述与 Scenario 测量边界）/ 150 ms transition / 100-200 ms 渐显区间 / 250 ms 动画上限 / 1500 ms long-wait 代表）。

**理由**：SPEC_GUIDE 反例 4 三分（用户感知 / 实现 tuning / 实测 baseline）已在两个姊妹 cap 验证。本 cap 复用规则不重新发明：用户能"看见 / 感知 / 区分"的阈值（toast 闪现时长、滚动停顿阈值、渐显感知区间、长时间等待视觉不升级）保留具体值；用户感知不到的 race 触发窗口（200-500 ms）+ 内部 debounce timer 实现细节移 design.md。
...
### D-6：实现 tuning 标题命名 follow-up

`Store loadMore 实现 leading + trailing debounce 100 ms` 与 `Store loadFirstPage / loadMore 内部 generation token 取消机制` 两个 Requirement 标题中嵌入了内部 store API 名 + tuning 数字，按严格 SPEC_GUIDE 命名应为「sidebar 加载更多 session 的高频限频」「sidebar 分页加载并发取消」之类领域语义。但标题 RENAMED 工艺成本（needs RENAMED Requirement op + 同步所有 reference）相对收益不划算（PR #319 D-2b 同款判定）。

**决策**：本 PR 不动标题，作为 follow-up 留 issue / 后续 spec 重组 PR 一起处理。
...
- **Trade-off**：D-2c 的 100 ms 是否算"用户感知阈值"边界争议。本 PR 选保留 —— body 自述「100 ms 是人类感知滚动停顿阈值」是直接的用户感知论证，与 600ms toast 同档；codex 二审若认为应严格抽象，apply 阶段反转代价低（删 Scenario 内 100 ms 即可）
