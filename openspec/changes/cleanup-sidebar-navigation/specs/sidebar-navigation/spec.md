...
`sessionListStore.loadMore(projectId)` API 自身 SHALL 实现 **leading + trailing 组合** debounce，让调用方在高频触发场景下（如未来 sidebar 把 `maybeLoadMoreSessions` 直接转发到 store）无需自己实现限频。

debounce 阈值 SHALL 取人类感知滚动停顿的阈值 100 ms（< 100 ms 视为连续滚动；> 100 ms 视为停顿）。该阈值是用户感知契约——用户能区分"持续滚动"与"已停下"，调用频率被合并到这两态而非更细粒度。
...
- **WHEN** 调用方在 100 ms 内连续多次调 `store.loadMore("A")`
...
- **WHEN** 第一次 `store.loadMore("A")` leading fire 后 fetch 已 resolve（不再 inflight）；接下来 100 ms cooldown 内调用方再调多次 `loadMore("A")`
