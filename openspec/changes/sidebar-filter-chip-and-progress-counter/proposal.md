...
**Spec fidelity**：本 change 同时把 `Sidebar.svelte:822` 单数字 `{visibleSessions.length}` 与 spec line 664 之间的脱节修正——之前 spec 写 `{visible}/{total}` 但实现退化到单数字，本 change 用更准确的 `{loaded}/{total}` 取代两边旧描述。
