...
#### Scenario: 点击"有更新"按钮锚定不丢失

- **WHEN** 用户在 `browsingHistory`（`scrollTop > HISTORY_SCROLL_THRESHOLD`）状态下点击"有更新"按钮触发 `refreshDeferredSessions` → `loadSessions(projectId, silent=true)`
- **THEN** 刷新完成后 `scrollTop` SHALL 保持不变；`scrollTop` 原本锚定的会话项 SHALL 仍存在于 `sessions` 数组中，DOM 节点 SHALL 被 `{#each (item.key)}` 复用
- **AND** Sidebar SHALL NOT 自动 `scrollTo({ top: 0 })`（保持 spec 既有"scrollTop 不重置"语义）
