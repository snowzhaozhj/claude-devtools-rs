...
系统 SHALL 实现 `get_worktree_sessions(group_id, pagination)` IPC：定位 `group_id` 对应 `RepositoryGroup`，把该 group 下所有 worktree 的 sessions 合并为单一列表，按 `lastModified` 倒序后再应用分页参数（`cursor` + `limit`）。返回 `PaginatedResponse<SessionSummary>`，每个条目 SHALL 额外携带 `worktreeId` / `worktreeName` 字段以便 UI 标注归属。

未命中 `group_id` 时 SHALL 返回 `not_found` 结构化错误（对齐 `Validate inputs and return structured errors` Requirement）。

#### Scenario: 合并多 worktree sessions 按时间排序
- **WHEN** caller 调用 `get_worktree_sessions("repo-1", { cursor: null, limit: 10 })`，repo-1 含两个 worktree 各 5 个 session
- **THEN** 响应 `items` SHALL 含 10 项，按 `lastModified` 倒序排列
- **AND** 每项 SHALL 含 `worktreeId` / `worktreeName` 字段

#### Scenario: 分页继续
- **WHEN** caller 接上一页 `next_cursor` 再调一次
- **THEN** 响应 SHALL 返回剩余 sessions，不重复返回上一页内容

#### Scenario: group_id 不存在
- **WHEN** caller 调用 `get_worktree_sessions("nonexistent-group", ...)`
...
- **WHEN** 前端调用 `invoke("get_worktree_sessions", { groupId: "g1", pagination: { cursor: null, limit: 20 } })`
