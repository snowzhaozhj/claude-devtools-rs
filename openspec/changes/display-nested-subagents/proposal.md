...
- `get_subagent_trace` IPC 在 `build_chunks` 之后调用该后处理,使返回给前端的子 transcript 把嵌套 `Agent` 调用暴露为可展开的骨架 subagent。`get_session_detail` 经 `parse_subagent_candidate` 构建 subagent candidate 内联 `messages` 时(`build_chunks` 之后)**同样**调用该后处理——非 Tauri 消费者 / HTTP route / 回滚裁剪开关时 subagent `messagesOmitted=false`,前端直接渲染内联 `Process.messages` 绕过懒拉,内联层嵌套 Agent 也须已升级(见 design D1b)。
...
- **零新文件 IO**:升级步骤只复用已解析出的 `result_agent_id`,不读取子文件、不额外 parse。展开一层 = 调一次今天已有的 `get_subagent_trace`,payload 与当前展开 level-1 完全相同路径。内联路径的 promote 进入 `get_session_detail` 主路径,但仅为已 build chunks 上的一次纯内存线性扫描(每 candidate 一次),零新 IO、不改 payload 结构(见 design D1b 性能验证)。
...
- **代码**:`crates/cdt-analyze`(新纯函数 `promote_result_agent_tasks` + 骨架工厂)、`crates/cdt-api/src/ipc/local.rs::get_subagent_trace` 与 `parse_subagent_candidate`(两处 `build_chunks` 后各调一次后处理)。前端 `ui/src/components/ExecutionTrace.svelte` / `SubagentCard.svelte` 预期零改(复用既有递归路径),仅补测试。
...
- **性能**:升级步骤无新 IO;内联路径 promote 进入 `get_session_detail` 主路径但仅为纯内存线性扫描(每 candidate 一次,实测 7f59237e 63 candidate 无可感知回归),不影响冷启动 / `get_session_detail` 预算。已知非阻塞 info:无 trace 级缓存,反复折叠重开会重复 parse 同一文件,留作后续可选优化。
