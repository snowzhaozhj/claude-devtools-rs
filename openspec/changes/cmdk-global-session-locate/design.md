...
## Apply 阶段修订（codex + pr-review 二审后，规则 7 三处同步）

### D8b：反转 D8——仅 debounce 后端 IPC，前端 filter/sort 走 raw query
D8 原定"query 驱动整段重算统一 debounce"。codex PR 二审 FINDING-2 指出：filteredProjects/selectedIndex 跟 `debouncedQuery` 会导致"输入后 250ms 内按 Enter 选到旧列表项"。复核成本后确认**前端 filter 极廉价**（候选 `allSessionRows` 已 `$derived` 随快照构建一次，每键只是 16k 字符串 `.includes` + 小结果排序，<1ms），真正贵的只有后端 `searchGroupSessions` IPC。故反转：
- 前端 filteredProjects / globalIdMatches / filteredSessions / sessionTotalBeforeCap / showShortQueryHint / actions / selectedIndex 重置 **全部走 raw `query`**（即时跟手）。
- `debouncedQuery` **仅**喂后端 `searchGroupSessions` IPC。
这正是 codex 初版二审 N1 推荐的"cache candidate + light filter"路径，且消除 FINDING-2。

### D7b：B 路结果按 (query, projectId) 双维打标，debounce 间隙/失败/切组不混入陈旧正文命中
codex FINDING-1 / silent-failure-hunter#1：raw query 变化后、新后端搜索返回前（debounce 间隙）或后端 reject 时，旧 `searchResults` 仍被 `contentMatchRows` 合并展示（stale-as-fresh）。修：`searchResults` 配 `searchResultsQuery` + `searchResultsProjectId` **双维**标记其归属；`contentMatchRows` 仅当 `searchResultsQuery === query.trim() && searchResultsProjectId === selectedProjectId` 才采用，否则返回空。后端 reject 时 seq 守卫清空 + 记当前 (query, projectId)。

**双维必要性**（codex 修复轮残留）：单维（仅 query）守卫在"**相同 query 切到另一组**"场景失效——旧组命中会显示到新组搜索 resolve。projectId 维度封住该跨组窗口。全局 id 定位走纯前端内存，后端搜索失败时仍可用（graceful degradation）。

### D4b：切组同步清空已加载会话，消除身份错配窗口
silent-failure-hunter#2：切组后 `listGroupSessions` 返回前（或 reject），旧组 `sessions` 仍在，空 query 分支会把旧组会话冠以新组 `groupId/projectName` 展示甚至错归属打开。修：select-effect 起始**同步** `sessions = []`，再异步加载；catch 加 seq 守卫清空。

### D5b：短查询提示独立渲染，不被 actions 撑非零的 totalResults 挤掉
codex FINDING-3：原提示门控 `totalResults === 0`，当 jobs action 存在时 totalResults≠0 → 未选项目的短查询不显示提示（违反 spec）。修：提示独立渲染，仅依赖 `showShortQueryHint`。

### D3b：加载失败与"无匹配"区分
silent-failure-hunter#3：D3 响应式读 store 后，`loadProjectData` 失败时空态显示"无匹配结果"误导用户。修：空态读 `getProjectDataError()` 区分"加载失败，请重试"与"无匹配结果"。

### D2c：无 title 回显完整 sessionId（不截断）+ 高亮匹配段
用户反馈：locate-by-id 时结果行只回显 `sessionId.slice(0,8)`（继承自旧 sidebar/palette fallback 约定），看不到完整 id 无法确认"找对会话"，且短前缀理论上有歧义。修：`rowTitle` 无 title 时返回**完整** `sessionId`（`.cp-item-label` 已有 ellipsis 兜底，36 字符在 520px 面板放得下）；并新增 `matchSegments` 把命中查询的子串包进 `<mark class="cp-match">` 高亮（复用 `--highlight-bg` token，与 Cmd+F 一致），title 命中同样高亮。tab 打开 label 同步用完整 id。
