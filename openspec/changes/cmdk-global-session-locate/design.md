...
### D7b：B 路结果按 query 打标，debounce 间隙/失败不混入陈旧正文命中
codex FINDING-1 / silent-failure-hunter#1：raw query 变化后、新后端搜索返回前（debounce 间隙）或后端 reject 时，旧 `searchResults` 仍被 `contentMatchRows` 合并展示（stale-as-fresh）。修：`searchResults` 配 `searchResultsQuery` 标记其归属 query；`contentMatchRows` 仅当 `searchResultsQuery === query.trim()` 才采用，否则返回空。后端 reject 时 seq 守卫清空 + 记当前 query。全局 id 定位走纯前端内存，后端搜索失败时仍可用（graceful degradation）。
