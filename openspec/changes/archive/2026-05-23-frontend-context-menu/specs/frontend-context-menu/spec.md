...
定义全应用右键菜单（context menu）的统一策略与基础设施：所有 surface 上的右键事件 SHALL 走 app 自家菜单或被静默拦截，永远不再漏到 WKWebView / macOS 系统菜单（Reload / Look Up / Translate / Search with Baidu / Speech / Services 等）。本 capability 提供 `AppContextMenu` 通用浮层组件、`use:contextMenu` Svelte action 与全局兜底监听器，作为后续各 surface 加菜单 items 的基础设施。同时规约 WKWebView 在右键 `mousedown` 阶段的 smart-selection 行为防护，消除"右键时光标下文字被自动选中"的违和体验。

本 capability 仅定义基础设施与全局策略契约；具体各 surface（会话项、tab、消息 chunk、工具结果、worktree chip 等）有哪些菜单项是各自 capability 的关注点。
