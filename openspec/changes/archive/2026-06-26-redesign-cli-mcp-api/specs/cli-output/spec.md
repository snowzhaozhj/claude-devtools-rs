...
**Reason**: `content_mode`（omit/overview/full）是「为省 token 逼多次调用」的反模式根源；turn 模型 + 服务端内置截断（tool output ≥5KB）+ `get_tool_output` 取全文替代之，默认即返回 AI 可直接分析的完整数据。

**Migration**: 用 `cdt session <id>`（默认 turn compact）+ `cdt turn <id> <n>`（完整 steps）+ `cdt tool-output`（全文）替代 `--content-mode` 各档；`--raw` 取原 chunk。
