...
**Reason**：dropdown 是 macOS WKWebView 风格的"打开 → 选 → 关闭"两步交互，与 sidebar 整体「Quiet Debugging Workbench」语言脱节；用户切 worktree 看不到全 wt 名（折叠在 dropdown 内），多 wt group 时尤其费眼力。本 change 用 chip cluster 替代——所有 wt 一眼可见 + 一次点击切换 + 单选语义不变。详见本 change `design.md::D1`。

**Migration**：单选 filter state（`worktreeFilter` `ALL_WORKTREES` / 具体 wt id）+ effect → loadSessions(filter) + buildFilterCursor 链路保持不变，**仅替换前端控件形态**。无 IPC / 持久化字段变化。新形态详见同 change ADDED `Worktree filter chip cluster for multi-worktree group` Requirement。
