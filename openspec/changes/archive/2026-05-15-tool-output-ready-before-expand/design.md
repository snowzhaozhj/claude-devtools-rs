...
- `WriteToolViewer.svelte`：成功态仅渲染 input.content，与 output 无关；失败态 Write 不走 WriteToolViewer，fallback 到 DefaultToolViewer 渲染错误详情，依赖 output。
...
- **不**改 Edit（任意 `isError`）与成功态 Write 的展开行为（它们不依赖 output；改了反而让用户感觉按钮无响应）。失败态 Write 走 DefaultToolViewer，按依赖 output 的路径处理。
...
**采用方案**：定义一个 `viewerUsesOutput(exec): boolean`，按工具实际路由的 viewer 决定：
- Read（任意 `isError`）/ Bash / DefaultToolViewer 路径（含 `isError=true` 的 Write —— fallback 到 DefaultToolViewer 渲染错误详情）：`true`
- Edit（任意 `isError`）：`false`
- Write 且 `isError=false`：`false`（走 WriteToolViewer 仅渲染 input.content）
...
**候选方案**：
- A：`if (exec.outputOmitted && !isOutputReady(exec))` —— 简单但会让 Edit 与成功态 Write 在 outputOmitted=true 时也等 IPC，按钮看起来卡（实际不需要 output）。
- B：`if (!isEditTool(exec) && !isWriteTool(exec))` —— 反向枚举，未来加新 viewer 或 Write 失败态 fallback 时容易漏。

**取舍**：选 D1（正向白名单）。`viewerUsesOutput` 与 `SessionDetail.svelte` / `ExecutionTrace.svelte` 的 ToolViewer 路由表保持同一判定语义；新增 viewer 时显式声明是否依赖 output。
...
### D2b（apply 阶段修订，回应 codex 二审 Bug 2）：抽 `shouldPrefetchOnChunkExpand` 让 SHALL NOT 契约可测

D2 原计划仅靠 `prefetchReadOutputs` 函数命名 + spec scenario 守护"AIChunk 展开不 prefetch Bash/Default"。codex 指出：未来一行小改（把 `isReadTool` 改成 `viewerUsesOutput`）能通过现有所有测试 + `openspec validate` 而悄悄破契约。修订：把判断抽成 `toolHelpers.shouldPrefetchOnChunkExpand(exec)`（仅 Read 非 isError 且 `outputOmitted=true` 命中），vitest 覆盖 Read 命中 + Bash/Grep/WebFetch/Write/Edit/Read isError 全不命中。`prefetchReadOutputs` 内部 import 这个 helper 调用，未来一旦改实现都要先改 helper，单测会立即报红。
...
**采用方案**：`ExecutionTrace.svelte::toggle` 与 `SessionDetail.svelte::toggle` 共用 `toolHelpers.viewerUsesOutput`，保证嵌套层（SubagentCard 内）行为一致。
...
- **[未来扩展]** 新增 ToolViewer 时需要在 `toolHelpers.viewerUsesOutput` 显式登记，并与 `SessionDetail.svelte` / `ExecutionTrace.svelte` 的 ToolViewer 路由表同步复核。
