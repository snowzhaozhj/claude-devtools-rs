...
`emit_json` 改为：`serde_json::to_string(value)` → `from_str` 成 `Value` → 递归脱敏得到 `(redacted_value, count)` → `count > 0` 时包 `{data: redacted_value, redacted: true, redactedCount: count}`，否则原样返回最初的序列化字符串（保留字段声明顺序、字节不变）。此核心抽为自由函数 `redact_and_serialize(&Redactor, value)` 以便单测 emit 层。
...
### D2b：撤销 D2，`password` 正则保持原 `\S+`（codex + silent-failure 二审 finding）

apply 阶段二审（codex PR review + silent-failure-hunter）双双指出：D2 的 `[^\s"]+` 排除 `"` 会在**密码值本身含双引号**时（叶子值 `password=ab"cd`）只脱掉 `ab`、泄漏 `"cd`——这是一处安全向的**欠脱敏**。

根因认识修正：`\S+` 当初危险，仅因旧实现在**序列化后的整串**上跑正则、`\S+` 跨叶子吃穿结构。D1 把脱敏改到**单个字符串叶子内部**后，`\S+` 匹配到叶子内下一个真实空白为止——最坏只是把该叶子的值多脱一点（over-redact，安全且更保守），永远不可能跨出叶子破坏 JSON。故收窄不仅多余，还引入欠脱敏。

结论：撤销 D2，`password` 正则恢复为原 `\S+`。JSON 结构安全完全由 D1 保证；宁可叶子内多脱，不可漏脱。
