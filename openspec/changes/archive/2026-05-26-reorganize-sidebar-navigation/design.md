...
- `sidebar-navigation` 主 spec 的 owner 边界改为用户行为视角，reviewer 可按「用户如何导航 / 查找 / 操作 / 切换 worktree / 调整 sidebar」定位契约。
- 保留或迁移的 Scenario 行为契约 100% 不变；迁入 `tab-management` 时允许把实现术语改写为用户可观察行为。
...
**问题**：本 change 涉及 36 个 Requirement 重组与 4 个 Tab owner 候选 Scenario 跨 cap 裁定，任何顺手改变可观察行为都会把“文档重组”变成行为变更；但部分迁入 Scenario 原文含实现术语，直接照搬会违反 SPEC_GUIDE。

**决策**：所有保留或迁移 Scenario 的可观察行为语义保持不变。允许变化仅限：cap owner、Requirement 标题、Scenario 标题、Requirement body 的用户视角组织语言、Purpose 元描述，以及把迁入 `tab-management` 的实现术语改写为用户可观察行为。

**校验**：D-3 中 4 个候选逐条裁定；人工抽样比对迁入表中的 3 个新增 Scenario；codex / spec-guide 二审重点检查是否漏迁、改写行为或保留实现术语。

### D-2：用户视角分组审计

**问题**：现有 36 个 Requirement 按实现演进顺序排列，列表分页 / SWR / SSE recovery / worktree chip / context menu 分散在全文件，难以从用户行为入口检索。

**决策**：本 change 只通过 active delta 迁移明确属于 `tab-management` 的 owner，并在 `sidebar-navigation` Purpose 中记录用户行为边界。Requirement 顺序不在 archive 后手工改动；下面的分组仅作为本次 owner 裁定的审计索引，后续若要重排主 spec 需另开 workflow 例外或等待 OpenSpec 支持同 cap 安全排序。
...
|---|---|---|---|---|
| `会话选择与 Tab 联动` | `点击会话打开 tab` | `tab-management::打开 session tab` | 迁移 | 标签页生命周期和当前聚焦窗格归 Tab 系统维护；迁入后标题改为“点击会话打开标签页”，Sidebar 只保留点击入口 |
| `会话选择与 Tab 联动` | `高亮跟随 focused pane 的 activeTab` | `tab-management::Sidebar 与 Tab 联动` | 迁移 | tab-management 已有当前窗格 / 活跃标签页高亮 owner；迁入后标题改为“高亮跟随当前窗格的活跃标签页” |
...
| `selectedGroupId 与 worktree id 分层维护` | `打开 session detail 用 worktree id` | `tab-management::打开 session tab` 或新增同 Requirement 内小节 | 迁移 | 详情加载身份属于 session tab identity；迁入后标题改为“会话详情按所属工作树加载”，Sidebar 保留列表与 group filter owner |
...
**结果**：4 个候选中 3 个在 active delta 迁入 `tab-management`，`无 active tab 时无高亮` 由 tab-management 既有 Scenario 覆盖而不复制。issue 指定的 L516 与 L1048 均保留 Sidebar owner；L999 只迁移其中“会话详情按所属工作树加载”，其余 Scenario 保留 Sidebar owner。

### D-5：spec delta 与 Purpose 例外工艺

`openspec` delta 不支持在同一 capability 中对同名 Requirement 同时 `REMOVED` + `ADDED`，因此不能用 active delta 表达同 cap Requirement 重排。本 change 采用保守工艺：

1. active change delta 中只用 `REMOVED` / `MODIFIED` / `ADDED` 表达可由 OpenSpec 验证的跨 cap owner 迁移：`sidebar-navigation` 移除 `会话选择与 Tab 联动`，并从 `selectedGroupId 与 worktree id 分层维护` 删除会话详情按所属工作树加载这一行为；`tab-management` 接收 3 个新增 Scenario，另 1 个候选由既有 Scenario 覆盖。
2. archive 仅执行 `openspec archive reorganize-sidebar-navigation -y` 的自动同步，不手工改 `## Requirements` 顺序。
3. 若 archive 后需要更新 `sidebar-navigation` 的 `## Purpose`，同一 archive commit 只改 Purpose 元描述：OpenSpec delta 不解析 Purpose；候选方案是保留旧 Purpose、直接改主 spec Purpose 或另开纯 Purpose change；本 change 选择同 archive commit 改 Purpose，因为它只描述用户价值与 owner 边界，不新增 SHALL/MUST 行为契约。

该工艺沿用 change `split-session-display` 的 D-6 先例对 Purpose 元描述的处理，并明确不把该例外扩展到 Requirement body 或顺序。
...
3. apply 阶段完成 active delta 中 4 个候选的 owner 裁定（3 个新增迁入，1 个既有覆盖）；同 cap sidebar Requirement 排序仅记录为 D-2 审计索引，不在 archive 后手工改主 spec。
...
6. 三路通过后 `openspec archive reorganize-sidebar-navigation -y`，同 archive commit 仅在必要时更新主 spec Purpose 与 spec-purity baseline。
