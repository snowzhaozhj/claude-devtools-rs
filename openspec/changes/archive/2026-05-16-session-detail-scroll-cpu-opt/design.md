...
### D2b: AI header 不放进 `content-visibility` 边界

实现阶段验证发现 `content-visibility: auto` 会启用 paint containment，即使显式 `contain` 只写 `layout style`，也可能裁剪 AI header 的 token popover 等溢出层。AI row 外层 SHALL NOT 使用 `content-visibility: auto`；仅对 `ai-tools-section` 与 `ai-body` 这类不需要 header popover 溢出的子区域启用渲染隔离。User / System / Compact 行仍可使用行级 `content-visibility: auto`。
