...
### D6b：apply 阶段修订——加载后校正"不改外层几何"收窄为"不放大外层几何"（PR 二审 test-analyzer 发现）

D6 原表述要求"无论最终档为完整内联 / 限高 / 超大，外层占位几何加载前后不变"。实测矛盾：最终档为完整内联（短内容 / missing）时，保持限高档 352px 占位意味着每个短输出永久挂一块空白——比收缩跳动更差的持续视觉噪音。**收窄**：bounded 占位 → bounded / oversized 最终态零跳变（原保证保留）；最终态为 inline 时允许收缩到自然高度。风险评估：收缩发生于 (a) 用户主动展开的交互点（视口内，用户预期内容变化）或 (b) tab 恢复补拉（伴随锚点滚动恢复机制兜底）；"视口上方静默收缩"要同时满足 tab 恢复 + 该项在锚点上方 + omitted + 最终 inline，组合概率低且有恢复机制。spec delta 已同步。
...
### D7b：apply 阶段修订——字符安全收窄到码点边界（PR 二审 test-analyzer 发现）

D7 原表述含"不拆组合字符簇"。实现（`takeBytesFromStart/End`）只保证码点边界；grapheme 边界需 `Intl.Segmenter` 对超大单行做切点回溯，复杂度与收益不成比：超大单行（≥256 KiB 无换行）几乎全是日志 / base64 / minified 产物，簇被拆的可见影响是 ZWJ emoji 显示为组成部分，不产生非法字节。**收窄**：SHALL 码点边界 + 不产生非法字节序列；簇完整性显式记为非目标。spec delta 已同步。
