...
- [ ] FU-3 invalidator root 切换协调：运行时 `update_general.claudeRootPath` 改 root 后，`spawn_parsed_msg_cache_invalidator` 持的 `projects_dir` 是构造时快照——已确认 `reconfigure_claude_root`（`local.rs:1305-1348`）会 abort + rebuild invalidator，无 stale 问题（codex 二审 design Q2 已 close）。本项保留作历史记录
- [ ] FU-4 SSH disconnect 中间态 error code 统一：当前 strict 变体返 `ApiError::not_found` 让 HTTP 映射 404 —— 复刻 PR-B 之前的 `active_fs_and_projects_dir()` 行为；若要让前端区分"SSH 临时 disconnect"与"资源真不存在"，开 PR 把 SSH-related error 切到专用 `ApiError::ssh(...)` 让 HTTP 映射 502（codex 二审 commit-stage round-3 Q2）。涉及 IPC error 契约 + 前端处理代码，跨 PR scope
- [ ] FU-5 前端 SSH disconnect 错误态展示：`ui/src/.../Sidebar.svelte` 与 `sessionListStore.svelte.ts` 当前 list IPC 失败时仅 `console.error` 清空列表，无 disconnect 重试 UI。开 PR 加 `sessionsError` 字段 + 列表区错误/重试组件。先做 FU-4 error code 统一再做（codex 二审 commit-stage round-3 Q3/Q4）
- [ ] FU-6 SSH disconnect 中间态 mock 单测：构造 `active_context_id` 返 Some 但 `provider_and_context_id` 返 None 的 fake `SshSessionManager` 场景，断言 `list_sessions` / `list_group_sessions` / `get_tool_output` 返 strict 错（codex 二审 commit-stage round-3 Q5）。本 PR 加了 `active_fs_and_context_strict` 的 1 个轻量 lib-internal unit test 兜底；端到端 mock SSH 测试留 FU 与 FU-4 一起做
