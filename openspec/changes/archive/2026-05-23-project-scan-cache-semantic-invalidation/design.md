...
伪代码（D2b 最终态：`has_entry || has_in_flight_scan` 双条件守护）：
...
match recv_result {
    Ok(event) => {
        let structural = {
            ...
            // 守护组合：cache 有 entry **或** 有 in-flight scan 时才走 unknown_session
            // 判定。前者覆盖稳态；后者覆盖"启动后第一次扫描期间结构事件"防漏。
            // 详 D2b 决策表 + spec `ProjectScanCache 按事件语义分级失效`。
...
- **[R8] `in_flight_scans` 跨 ContextId 计数粒度（codex PR 二审第三轮 WARN）**：当前 `ProjectScanCache.in_flight_scans` 是全局计数，不区分 `FsKind::Local` / `FsKind::Ssh`。Local watcher 事件触发 `invalidate_local()` 同时 bump `invalidation_generation`——若此时**SSH** scan 正好在途（`begin_scan` 已加 1 但 `finish_scan_with_insert` 未到），SSH finish 会因 generation mismatch 丢弃本次 SSH snapshot，下次 SSH lookup miss 再扫一次。
  → **正确性**：不破坏正确性——caller 仍能拿到本次 scan 的 `Arc<Vec<Project>>` 返回值（finish 只决定是否落 cache）；且 `invalidate_local()` 不动 SSH `entries`（仅 `FsKind::Local` retain）。问题仅在于 SSH 端"不必要重扫一次"。
  → **接受 + Non-Goal lock**：本 change 已在 D5 显式 Non-Goal "不动 SSH backend"；R8 修复（按 `FsKind` 拆分 `invalidation_generation` + `in_flight_scans` 双桶）属 SSH 优化范畴，触发场景需 SSH 用户活跃使用 + 本地 fs 事件密集——边际场景。
  → **未做（留 followup）**：未来 SSH perf 优化 PR 中按 `ContextId` 或 `FsKind` 分桶 generation；现状先保 fail-safe 行为。
