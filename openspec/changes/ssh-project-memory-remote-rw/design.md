...
| **SSH 远端 rename 不原子**（非 OpenSSH 或老版 OpenSSH 不支持 `posix-rename@openssh.com`） | Mitigated by D2 探测 server extensions：支持则真原子路径，不支持降级为 `remove_file(<target>) + rename(<tmp>, <target>)` 两步——降级路径有极短窗口 reader 见 `target missing`，单次写场景 acceptable；本 change 不要求 caller 据此分支 |
