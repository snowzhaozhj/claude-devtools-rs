# mcp-server spec delta: mcp-server-stdio
...
`cdt setup mcp` SHALL 打印注册配置片段。`cdt setup mcp --apply` SHALL 读取 `~/.claude/mcp_servers.json`，merge `cdt` server entry（保留已有配置），原子写入（写 tmp 文件 → rename）。

#### Scenario: setup mcp 打印配置

- **WHEN** 用户运行 `cdt setup mcp`
- **THEN** SHALL 输出 JSON 配置片段（含 command、args）到 stdout

#### Scenario: setup mcp --apply 写入全局配置

- **GIVEN** `~/.claude/mcp_servers.json` 已有其他 server 配置
- **WHEN** 用户运行 `cdt setup mcp --apply`
- **THEN** `cdt` entry SHALL 被添加到文件中
- **AND** 其他已有 server 配置 SHALL 保持不变
