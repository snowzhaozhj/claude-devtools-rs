...
系统 SHALL 对每个 distro 执行 `wsl.exe -d <distro> -- sh -lc 'printf %s "$HOME"'` 解析 distro 内 `$HOME`，stdout 按"解码 `wsl.exe` stdout"规则解码后跑 `normalize_wsl_home_path`。

`normalize_wsl_home_path(input: &str) -> Option<String>` 行为：

1. trim whitespace
2. 若不以 `/` 开头 SHALL 返回 `None`
3. 按 posix 风格 normalize（解析 `..` / 折叠多 `/`）
4. 去除末尾 `/`（除非整路径就是 `/`）

**USERNAME fallback**：若 `normalize_wsl_home_path` 对命令输出返回 `None`：

- 若 Windows 进程环境变量 `USERNAME` 非空 SHALL 用 `"/home/" + USERNAME` 作为输入再跑一次 `normalize_wsl_home_path`，命中即视作该 distro 的 `homePath`
- 若 `USERNAME` 为空 / fallback 仍 `None` SHALL 视该 distro home 解析失败（计入 `distrosWithoutHome`）
...
- **AND** SHALL 记录 warn 日志说明 timeout
