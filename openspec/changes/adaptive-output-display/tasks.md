...
- [x] 2.4 限高 viewport 作为 lazy markdown 节点的**外层稳定容器**（codex W6）：min-height 占位在内层（`.lazy-md`）、`max-block-size`+滚动在外层（`.ao-viewport`）；字节触发路径占位估算 ≥ viewport 上限 → 外层恒钳在上限（`lazyMarkdown.test.ts` 不变量测试锁定）

## 3. AI prose 输出路径接入（核心痛点，仅两档不切片）

- [x] 3.1 `SessionDetail.svelte` 的 output / user_message prose 接入自适应框架（prose 轻量 variant）：**完整内联 / 限高预览两档，不做 top/tail 切片**（限高下完整内容留 DOM 保 Cmd+F 全文命中）；限高用 CSS block-size 不改 DOM 结构
- [x] 3.2 顶层主 AI 回复 lastOutput **保持完整内联不限高**（用户拍板，见 design D10；不接入限高）
- [x] 3.3 嵌套 ExecutionTrace（SubagentCard/WorkflowCard）内 output/user_message prose 同受两档限高（codex W10：该路径同步 renderMarkdown 不经顶层 lazy observer，须显式覆盖）
- [x] 3.4 验证搜索 hydrate（`ui-search` flushAll）后限高仍生效、长 prose 唯一关键词在中段仍命中（Playwright `adaptive-output-display.spec.ts`）

## 4. 工具查看器接入（三档含 top/tail）

- [x] 4.1 内容源矩阵（codex C4）：Read/Bash/Default 按 output、Write 按 input.content、Edit/diff 按 old/new、error 附 errorMessage
- [x] 4.2 `OutputBlock.svelte`（Bash/Default/Edit-result）接入三档 + 常驻复制全文，替代 hover-only overlay
- [x] 4.3 ReadToolViewer / WriteToolViewer 接入三档，与既有轻量高亮降级叠加
- [x] 4.4 DiffViewer（`edit-diff-view`）接入三档限高 + 信息气味（diff 行不做重高亮的既有约束保留）
...
- [x] 5.1 自适应框架 header 用常驻"复制全文"，复制完整原文；各输出面全文来源落表（prose→全文/Read·Bash·Default→output/Write→input.content/Edit·diff→差异，DiffViewer 新增"复制完整差异"）；更新 `copy-to-clipboard` 相关组件与测试
- [x] 5.2 完整原文未就绪（懒加载中/失败/Missing/空）时复制入口禁用 + 原因标签（AdaptiveOutputFrame/Read loading 态禁用 + aria 原因；missing 缓存标记终结占位态），SHALL NOT 降级复制可见切片；不引入新失败反馈（沿用既有静默降级，codex W9）
