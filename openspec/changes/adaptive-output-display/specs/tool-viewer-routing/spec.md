...
- **加载后校正不改外层几何**：懒加载到达后按真实内容规模确定最终档位；无论最终档为完整内联 / 限高 / 超大，内容 SHALL 在同一稳定的外层限高 viewport 内填充，外层 viewport 的占位几何 SHALL NOT 在加载前后改变——仅其内部内容变化，使视口上方的块加载完成时 SHALL NOT 改变主滚动几何。
...
- **字符安全**：切分点 SHALL 落在字符（Unicode 码点，且不拆分组合字符簇）与行边界上，SHALL NOT 在多字节字符中间截断产生乱码。
