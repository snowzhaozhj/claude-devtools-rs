...
### D1f（修订采纳）：Windows 退化为 `FileIdentity::None`

CI 编译失败发现：`std::os::windows::fs::MetadataExt::file_index()` 与 `volume_serial_number()` 是 unstable feature `windows_by_handle`，stable Rust 不可用。原 D1b 规划的 Windows `(volume_serial, file_index)` 维度无法在 stable toolchain 实现。

候选方案：
- A（**采纳**）：Windows 退化为 `FileIdentity::None`，与"其它平台"共享同一退化 variant，仅依赖 mtime+size。Unix 仍保留 `(dev, ino)` 维度。
- B：引入 `windows-sys` + `unsafe` FFI 调 `GetFileInformationByHandle` 拿 `BY_HANDLE_FILE_INFORMATION.{nFileIndexHigh|Low, dwVolumeSerialNumber}`。
- C：用 third-party crate（`same-file` / `winapi-util`）。

选 A 因为：
- D1d 已经把 inode reuse + mtime/size 撞车场景列为 best-effort，Windows 上完全没有 identity 维度也是同等 best-effort 的延伸——只是 Windows 更早一步落入 best-effort 路径
- B 引入 unsafe FFI + `windows-sys` workspace dep，成本远大于 Windows 上 inode 检测的边际收益（D1d 已评估）
- C 引入新依赖，且这些 crate 内部也是包装 GetFileInformationByHandle，与 B 等价的间接代价
- 现实中 Claude Code 在 Windows 上不主动 delete-then-recreate session JSONL，rename 替换 JSONL 的极端场景由后续 file-change 自然恢复

实施层影响：
- `FileIdentity` enum 简化为 `#[cfg(unix)] Unix { dev, ino }` + 共享 `None` variant（带 `#[allow(dead_code)]` 让 Unix 平台编译过——`None` 在 Unix 不被 `from_metadata` 构造，但仍需保留以让跨 cfg 测试代码引用）
- `from_metadata` Windows + 其它平台合并为 `#[cfg(not(unix))]` 分支返回 `Self::None`
- spec delta 两处 Scenario 标记为"仅 Unix"，并加注 Windows + 其它平台由 mtime/size 兜底
- 单元测试 `dummy_sig` / `dummy_entry` 的 cfg 分支同步合并
