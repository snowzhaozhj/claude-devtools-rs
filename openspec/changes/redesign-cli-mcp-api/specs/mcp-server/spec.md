...
**Reason**: `get_session_chunks` 工具被删除；其按 chunk 粒度的 `max_tokens` 预算截断被 `session-turn-view` 的服务端内置截断（tool output ≥5KB）+ step 分页 + `get_tool_output` 取全文替代。

**Migration**: 用 `get_turn` 取单 turn（自带 step 分页与 output 截断），用 `get_tool_output` 取被截断的完整原文，不再需要 `max_tokens` 参数。
