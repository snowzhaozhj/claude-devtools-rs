...
- `context-tracking`: turn 锚点语义从 AIChunk-anchored 改为 user-message-anchored；显式补上"被打断的 turn（无 AI group）仍产出 `UserMessage` injection 并占一个 turn"的契约；放宽 `aiGroupId` 不变量以容纳被打断 turn 锚到 `UserChunk.chunkId`。

## Impact

- **代码**：`crates/cdt-analyze/src/context/session.rs`（主循环 turn_index / user chunk 配对）、`crates/cdt-analyze/src/context/aggregator.rs`（`create_user_message_injection` 接受被打断 turn 的锚 id）、`crates/cdt-analyze/src/context/stats.rs`（参数透传）。
