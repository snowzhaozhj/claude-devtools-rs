...
**用户感知**：sshd hang 60s（实际 18s 触发）后系统**SHALL**走 `perform_polling_self_heal_disconnect` 自动 disconnect SSH context + emit `ContextChanged(None)` 事件，前端 listener 自动切回 Local UI——避免现状"sidebar 残缺 + 30s curl timeout"。**显式 follow-up（不在本 change 范围）**：用户向感的"SSH 已死"toast / 状态栏提示、自动重连尝试——这两项需独立 IPC 事件契约 + 前端 UX 设计，作为后续独立 change 处理。
