...
- **THEN** 函数体 SHALL 调 `fs.open_read(path).await` 拿 `Box<dyn AsyncRead + Send + Unpin>`，并用 `BufReader::with_capacity(SCANNER_BUF_BYTES /* 64 KiB */, reader)` 包装后逐行 `next_line().await` 喂 `IsOngoingStateMachine` / 标题 / 计数 / git branch 状态机
...
SSH list 路径**hot path（用户感知）SHALL 走 cache hit trust**：UI 立刻拿 in-memory cache 内容渲染列表（0 fs op），不等 fs.stat RTT；后台 spawn task 走 `fs.read_dir_with_metadata(project_dir)` per project（1 RTT 拿全 dir metadata，SFTP READDIR reply 自带 entry attrs），逐条比对 cache signature → mismatch / 新增 → spawn cache miss scan → 通过 SSE event 推差量给 UI 增量更新（与 Local 现有 SkeletonThenStream 体验一致）。**朴素 per-session 串行 fs.stat 路径 SHALL NOT 出现在 SSH list hot path** —— `Arc<Mutex<SftpSession>>` 全锁串行（PR-A D3 已知假 batch）会让 50 sessions × 50ms = 2.5s 直接超 sidebar 首屏预算。

SSH cache miss 路径产 1 个 `fs.read_dir_with_metadata`（per project，dir 内全部 entry metadata 一次拿到）+ 1 个 `fs.open_read`（per需 scan 的 session）。具体 fs op 计数：

- Hot path cache hit（用户切回已访问 SSH host）：UI 渲染 fs op = 0；后台 batch 校验 fs op = M `read_dir_with_metadata`（M = 页内 project 数）
- Cold start 首次连 SSH：UI 渲染骨架 fs op = M `read_dir_with_metadata`；spawn cache miss scan per session 走 `fs.open_read`，metadata 通过 SSE 推差量；典型 5-10 projects/page 250-500ms，重 user 30+ projects 约 1.5s（真消除留 PR-F SFTP message-id pipeline）

新加 cache helpers：
- `MetadataCache::lookup_with_known_signature(&mut self, ctx, path, sig) -> Option<&MetadataCacheEntry>`：用调用方提供的 signature 直接查 cache，跳过内部 stat（用于后台 batch 校验路径）
- `MetadataCache::lookup_trust_cached(&mut self, ctx, path) -> Option<&MetadataCacheEntry>`：hot path cache hit trust，不校验 signature 直接返 entry（UI 立刻渲染用）
...
#### Scenario: SSH 后台 batch 校验路径走 read_dir_with_metadata + SSE 推差量

- **WHEN** UI 渲染完 cache hit trust 内容后（或 cache miss 时启动）
- **THEN** SHALL spawn 后台 task 走 `fs.read_dir_with_metadata(project_dir).await` per project（SFTP READDIR reply 含 entry attrs，1 RTT 拿全 dir metadata）
- **AND** task SHALL 用 `cache.lookup_with_known_signature(&ctx, path, &sig)` 比对调用方已知 signature；mismatch / 新增 entry → 走 cache miss + scanner（`fs.open_read`）重 parse；结果通过 SSE event 推差量给 UI 增量更新
- **AND** task SHALL NOT 阻塞 list_sessions IPC 响应——hot path 走 cache trust 立即返回；后台 task 异步更新通过 SSE channel
- **AND** task SHALL 注册 abort handle 到 `LocalDataApi::active_scans` per-key cancel map；`ssh_disconnect` 时旧 ssh ctx 下所有 batch task SHALL 被 abort

#### Scenario: 冷启动 SSH list_sessions（cache 无 entry）

- **WHEN** 用户首次连 SSH host A 调 `list_sessions`，cache 中无该 ContextId 的 entry
- **THEN** SHALL 走 `fs.read_dir_with_metadata(project_dir)` per project 拿骨架 + metadata（M projects × 1 RTT）
- **AND** SHALL 通过 SSE event 推骨架 + 增量 metadata 给 UI；前端按 SkeletonThenStream 模式先渲染 file_name 骨架后填充 metadata
...
- Hot path cache hit（同 session 二次访问，外部未改）：UI 路径走 `cache.lookup_trust_cached(&ctx, path)` 0 fs op；后台 spawn task 走 `fs.stat(path)` 1 RTT 校验 signature，mismatch 时走 cache miss
...
新加 cache helpers（与 metadata cache 同型）：
- `ParsedMessageCache::lookup_with_known_signature(&mut self, ctx, path, sig) -> Option<&ParsedMessageEntry>`：用调用方提供的 signature 直接查
- `ParsedMessageCache::lookup_trust_cached(&mut self, ctx, path) -> Option<&ParsedMessageEntry>`：hot path 不校验 signature
