...
- [x] 1.2 调整 ReadToolViewer，使较大输出展开时降级轻量高亮，小/中等内容保留完整 `highlightCode`。
- [x] 1.3 调整 WriteToolViewer，使较大内容展开时降级轻量高亮，小/中等内容保留完整 `highlightCode`。
- [x] 1.4 调整 DiffViewer/EditToolViewer，使 diff 行保留统一 diff 语义但不执行逐行重型语法高亮。
- [x] 1.5 检查 SessionDetail 展开状态与输出缓存路径，减少与单个工具无关的派生重算，并让 omitted Read 输出 ready 后再展开。
