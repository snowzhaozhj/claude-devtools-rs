...
- 将 4 个 Tab owner Scenario 从 `sidebar-navigation` 迁到 `tab-management`，避免 Sidebar 与 Tab 双 owner。
...
**问题**：本 change 涉及 36 个 Requirement 重组与 4 个 Scenario 跨 cap 迁移，任何顺手改写 WHEN / THEN / AND 子句都会把“文档重组”变成行为变更。
...
**校验**：迁移前后 Scenario 总数守恒；人工抽样比对迁移表中的 4 个 Scenario；codex design 二审重点检查是否漏迁或改写子句。
...
| `会话选择与 Tab 联动` | `无 active tab 时无高亮` | `tab-management::Sidebar 与 Tab 联动` | 迁移 | 与 tab-management 既有同名语义重叠，应单 owner |
...
**结果**：只迁移 4 个 Scenario。issue 指定的 L516 与 L1048 均保留 Sidebar owner；L999 只迁移其中 `打开 session detail 用 worktree id`，其余 Scenario 保留 Sidebar owner。
...
1. active change delta 中只用 `REMOVED` / `MODIFIED` / `ADDED` 表达可由 OpenSpec 验证的跨 cap owner 迁移：`sidebar-navigation` 移除 `会话选择与 Tab 联动`，并从 `selectedGroupId 与 worktree id 分层维护` 删除 `打开 session detail 用 worktree id`；`tab-management` 接收这 4 个 Scenario。
2. archive 后在同一个 archive commit 内直接编辑主 spec 的 `## Purpose` 段与 Requirement 顺序，完成 D-2 的用户行为分组排序；Purpose 不含新的 SHALL/MUST 行为契约，排序不改变 Scenario 子句。
...
| 重组遗漏 Scenario | 高 | 迁移前后 scenario 总数守恒；D-3 显式列 4 个迁移 Scenario；spec-guide-reviewer + codex 二审 |
...
3. apply 阶段完成 36 个 Requirement 重组 delta 与 4 个 Scenario 迁移。
4. 运行 `just preflight`、`bash scripts/check-spec-purity.sh`、Scenario 数守恒校验。
