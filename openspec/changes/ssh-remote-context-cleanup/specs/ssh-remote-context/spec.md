...
系统 SHALL 通过 SSH 连接到远程主机，连接时 SHALL 在 `~/.ssh/config` 存在的情况下读取主机元数据；SHALL 支持显式断开与应用退出时的优雅断开。连接 SHALL 走真协议栈（非占位实现），按以下五个阶段顺序进行：TCP probe → SSH transport 握手 → 鉴权候选链尝试（详 `Requirement: SSH authentication candidate chain`）→ SFTP subsystem open → remote home probe；每阶段与外层总流程 SHALL 各有独立硬超时。任一阶段失败 SHALL 返回结构化 `SshError`（详 `Requirement: Structured SSH error classification`）。
...
- **AND** 全部不存在时 SHALL 返回 `SshError::RemoteHomeMissing { tried }` 错误，状态切到 `error`，不切换 active context，但状态查询 SHALL 保留该 context 的错误状态与已完成的 `authChain` 诊断
...
系统 SHALL 在 SSH context 处于 `connected` 状态时启动远端文件变更感知 polling watcher：以固定间隔轮询 SFTP `read_dir(<remote_home>/.claude/projects/<project_id>/)` + 对每个 `.jsonl` 文件 `stat` 取 size 与 mtime，与上轮快照比较差异（新增 / size 变化 / 删除）后通过与本地 watcher 相同的 `FileChangeEvent` schema 广播事件。第一次 poll SHALL 不触发任何事件（建参照快照用）。系统 SHALL 额外定期运行 catch-up 比较作为兜底。SHALL 在 `ssh_disconnect` 时停止 watcher 与释放 SFTP 资源。

watcher SHALL 把每轮 poll 失败按错误特征分到三类（在 polling 层 `with_retry` 之后做语义升级，**不**改 SFTP client trait 错误分类）：
...
- 永久失败连续计数 `consecutive_permanent`，达永久失败阈值后触发 dead signal
- 超时连续计数 `consecutive_timeout`，达超时阈值后触发 dead signal

超时阈值 SHALL 远高于网络瞬时抖动 window，且远低于用户感知 sidebar 僵死的主观放弃阈值。
...
- **THEN** 该 context 的 polling task SHALL 在短时间内退出（cancellation token）
...
- **WHEN** SFTP `read_dir` 连续多轮 poll 都返 `Transient("timeout")` 类错误（典型场景：远端 SFTP 协议层 hang 但 TCP 未断）
- **THEN** watcher SHALL 在 `consecutive_timeout` 达阈值的那一轮通过 dead 信号通知并跳出主 loop
- **AND** 触发上层 monitor task 走自愈断开把 active context 切回 `Local`
- **AND** 总耗时上限 SHALL 显著低于用户主观放弃阈值

#### Scenario: 永久 transport 错误达阈值后触发 dead 信号

- **WHEN** SFTP `read_dir` 连续多轮 poll 都返含 `session closed` / `broken pipe` / `connection reset` 等 transport-dead 关键字的错误（无论错误变体来源）
- **THEN** watcher SHALL 在 `consecutive_permanent` 达阈值的那一轮通过 dead 信号通知并跳出主 loop
...
**bump-first 顺序契约**：5 处路径 SHALL 在 `ssh_mgr.connect / switch_context / disconnect` / `ssh_mgr.shutdown_all` / 写新 `projects_dir` 这一步 await **之前** 完成 context generation 自增（`reconfigure_claude_root` 同时自增 root generation）。理由：保留这一顺序让任何 in-flight `list_sessions_skeleton` / `build_group_session_page` 在 spawn 时记录的"期望 generation"立即失效（broadcast 前 check `current != expected` → silent drop）。如果反转为"先 await 后 bump"，await 期间 ssh_mgr 状态部分切换但 generation 未 bump，in-flight scan task 的 broadcast 校验仍通过，会向前端串扰旧 ctx 的 metadata update。
...
- **AND** 锁内查询 `ssh_mgr.active_context_id()` SHALL 返回 `None`（Local active）；重建的 ContextId = `Local { projects_dir }` 与 captured_ctx = `Ssh<host_a>` mismatch
...
SSH session manager SHALL 暴露 `context_id(&str) -> Option<ContextId>` 查询方法：

- 入参为已注册 SSH context 的 context_id 字符串
- 命中时 SHALL 从 SSH session 资源取 host_signature 与 remote_home，合成 `ContextId::ssh(host_signature, remote_home)` 返回 `Some(_)`
- 未注册（含已 disconnect / 未连接成功）时 SHALL 返回 `None`
- SHALL NOT 调用 `ssh -G` resolve 子进程

SSH session manager 的 test helper SHALL 接受可选 host signature 参数；缺省时 SHALL 用 `(host, port, user)` 字符串拼接做 fake digest 构造一个 host signature，使不同 host 的测试 fixture 自然产不同 digest。

SSH session manager SHALL 暴露原子查询方法 `provider_and_context_id(&str) -> Option<(SshFileSystemProvider, ContextId)>`——单次内部状态 lock 内同时返回 provider 与 ContextId，保证二者来自同一快照。调用方 SHALL 用本方法而非独立的 provider + context_id 配对取 fs/ctx，避免 disconnect race 产生 `(SSH provider, Local ctx)` 不自洽组合。
...
- **THEN** 实现 SHALL 调 SFTP client trait 的 `read(path)` 方法拿全量 bytes
- **AND** SHALL 返 Cursor 包装的全量 bytes
...
- `create_dir_all` SHALL 通过 SFTP 递归创建目录，对每段父目录先调 `try_exists` 探测，已存在跳过；缺失调 mkdir 创建。任何 SFTP rpc 失败 SHALL 走既有 retry 策略
...
系统 SHALL 在每次 `ssh_connect` 建立 SSH client 时启用 transport 层 keepalive，配置为以固定间隔距离上次 server 数据后发一次 keepalive 请求（要求 reply），由 SSH 库内部 keepalive loop 在累计未应答 tick 数超过最大容忍数后主动关闭 transport（注意底层库当前实现的 off-by-one 语义，实际触发窗口比标称值多一轮 interval）。

实现 SHALL 暴露 keepalive interval 与最大容忍 tick 数两个常量，并 SHALL 通过统一的 client config 构造 helper 把这两个常量写进 SSH client config。connect 阶段 SHALL 调用该 helper 而非默认 config 构造握手；helper 里其它字段 SHALL 通过结构体更新语法保留 SSH 库默认值（SSH client config 不实现深比较语义，一致性由构造方式保证而非运行时断言）。

启用该机制的目的：(1) **主作用**：每个 keepalive 间隔让 channel 双向有 SSH msg 流动，防止 server-side 默认禁用 keepalive / NAT idle / firewall idle 把 channel 静默关闭；(2) **次作用**：让对端硬故障（拔网线 / SSH 服务重启）能在合理时间内被 client 主动发现，由 SSH 库关闭 transport，触发既有 polling watcher → dead 信号 → 自愈 disconnect 链路（详 `Requirement: Watch remote project directories via SFTP polling`）。
...
- **THEN** 返回的 SSH client config SHALL 满足 keepalive interval 设为约定常量 且 keepalive 最大容忍 tick 设为约定常量
- **AND** 实现 SHALL 通过 `Config { keepalive_interval, keepalive_max, ..Default::default() }` 语法构造，确保其它字段从库默认继承（不引入额外副作用）
...
- **AND** transport 握手成功后 SSH 库内部 keepalive loop SHALL 按约定间隔周期运行（由库内部实现保证，本契约只钉死 client config 入参）
