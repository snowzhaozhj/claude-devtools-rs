...
`LocalDataApi::new_with_watcher(...)` 构造路径 SHALL spawn 后台 task，订阅 `FileWatcher::subscribe_files()` 广播。该 task 对每条 `FileChangeEvent` SHALL 仅根据 `event.project_list_changed` 与 `event.deleted` 两个字段决定是否失效 `ProjectScanCache` Local entry：
...
2. `event.session_id` 非空 **AND** `ProjectScanCache::contains_session_id(local_ctx, &event.project_id, &event.session_id) == false`（即 cache snapshot 未包含此 session）→ 同规则 1：`invalidate_local()` + structural counter
3. 其他（普通 JSONL append + watcher 折叠的 subagent 修改 + 空 sid 事件）→ **不**调任何失效 API，保留现有 cache，inc counter `project_scan_cache.invalidate.content_append_skipped`
