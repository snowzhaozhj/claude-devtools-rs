...
- [x] 1.3 在 `ui/src/routes/SessionDetail.svelte` `import { viewerUsesOutput }`，把 `toggle` 中 `isReadTool(exec)` gate 替换为 `viewerUsesOutput(exec)`，并移除原 `if (exec && !isReadTool(exec)) { void ensureToolOutput(exec); }` 兜底
...
- [x] 2.4 vitest 覆盖 `viewerUsesOutput(Write isError) === true`、`viewerUsesOutput(Read isError) === true`（这两类走 DefaultToolViewer 渲染错误详情，仍需要 output）。`prefetchReadOutputs` 仅过滤 Read 的契约靠 spec scenario "展开 AIChunk 不主动 prefetch Bash 与 Default" + 函数命名守护，不引入额外 helper
