...
**inline prompt only**——`bg-pr` recipe 用 just `quote()` 把 NAME / PROMPT 编码为 shell-safe 单引号字面量，含 backtick / 双引号 / `$` / `$HOME` 等特殊字符的 prompt 也能原样传入，不被宿主 shell 解释（change `unify-fs-direct-calls` 修订）：
...
2. **inline prompt only**——禁止落 prompt 文件作 escape hatch（见上节"禁止落 prompt 文件"）
