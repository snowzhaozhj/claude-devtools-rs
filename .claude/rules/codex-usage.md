...
codex 是**异构推理**（不同模型查逻辑盲点），pr-review-toolkit 是**同模型多视角**（专项 agent 查特定维度）。二者并行不替代。

| 维度 | codex | pr-review-toolkit |
|---|---|---|
| 逻辑 bug / 非法状态序列 | ✓（核心） | — |
| 静默失败 / catch-and-swallow | 部分（#8） | `silent-failure-hunter` |
| 测试质量 / 覆盖盲点 | — | `pr-test-analyzer` |
| 类型设计 / 封装 | — | `type-design-analyzer`（可选） |
| 代码简化 / 冗余 | — | `code-simplifier`（业务段 pre-commit） |

**不调**：`code-reviewer`（与 codex + 域专审重叠）、`comment-analyzer`（项目策略默认不写注释）。

触发条件与调用方式详见 `opsx-apply-cadence.md::发布尾段 step 9`。

## 调用记录

每次调完说一句 "已让 codex 二审，找到 N 个 bug / 0 个问题"。pr-review-toolkit 同样报结果，如 "pr-review-toolkit: silent-failure-hunter 0 问题 / pr-test-analyzer 2 建议"。
