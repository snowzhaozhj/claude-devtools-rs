...
## Lead 起 team 前 SHALL 完成（环境前置 + 集中职责）

teammate 文件硬约束写了"分支 / worktree / commit / push 由 lead 保障"——这条对 lead 的反向责任：

**起 team 前**：

1. **fetch + 切分支**：`git fetch origin main && git checkout -b feat/<slug>`（**禁止**在 main 直接干；hook 会 deny 但你别等 hook）
2. **EnterWorktree**：切到 `.claude/worktrees/<branch>/` 隔离环境——4 个 teammate 同时写主仓会污染你的工作目录与 LSP / hot-reload 状态
3. **propose 骨架就绪**：design.md（含 D 决策）+ spec delta + tasks.md 至少有骨架；teammate 看的就是 design.md / tasks.md，骨架空着 team 起来也没头绪（视觉契约 `D-V<n>` + `## Visual Contract` 段由设计师 teammate 在 propose 阶段并行补，但其它 D 决策与 tasks 拆分 SHALL 你先有雏形）
4. **`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 已生效**：shell env 或 `settings.json` 任一即可

**起 team 后**：

5. **commit / push 你独占**：teammate 改完文件 SendMessage 你 → 你 `git status` 检查 → 你 `git add` + `git commit` + `git push`。**禁止**派 teammate 自己 commit（破坏 commit 颗粒度 + 难审 PR）
6. **archive 你独占**：`openspec archive` 是 lead 职责，不派给任何 teammate
7. **5 个 reviewer subagent + codex-rescue 你独占**：teammate 触发条件命中（详各 teammate 文件 `## 工作流` 段）时 SHALL SendMessage 你来调，不让 teammate 自己 spawn subagent
8. **环境异常你独占**：teammate 报"git 状态意外"（如发现在 main / worktree 损坏） → 你立即查环境是不是哪一步跑歪，**不要**让 teammate 自己 reset / checkout

**Mid-apply 升级路径**：详 `.claude/rules/opsx-apply-cadence.md::形态升级判断`。
