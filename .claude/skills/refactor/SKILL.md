...
只有以下 4 条**全部**通过才允许把 boundary 项降级为纯结构改路径。**默认每条都不通过**——降级是 fail-safe 的反向操作，需要**显式证据**逐条 satisfy，模型不允许"凭直觉"判断 OK。

- **测试覆盖**（machine-verifiable gate）：被改 API / 边界有现存测试，且 `cargo test -p <crate>` 改前 + 改后两次都全 pass，并贴出测试名 list 或输出。**无现存测试 = 不通过**（先补测，不能"我认为行为没变"）
- **callsite grep**（machine + manual gate）：先 `grep -rn '<被改符号>' --include='*.rs' --include='*.svelte' --include='*.ts'` 列出所有 caller 文件:行号——这是机器可验证步骤；然后**逐 caller 显式说明**它依赖了被改的哪些语义（参数顺序 / 返回类型 / lifetime / Send + Sync bound），并解释为什么改动不破坏。**caller > 5 处或 cross-crate caller 存在 → 默认不通过升级 openspec**（人工逐 caller 论证不可信）
- **IPC contract test 不动**（machine-verifiable gate）：`cargo test -p cdt-api --test ipc_contract` 跑过且无字段差异；**仅当涉及 IPC 时该条适用**——非 IPC 改动跳过本条不算"不通过"
- **codex diff 自审**（异构二审，不信任自己的 diff 自审）：调 `Agent({ subagent_type: "codex:codex-rescue" })` 让 codex 看 diff 评估"这是不是只改了实现没改契约"。**SHALL 用 codex 不能自己看自己 diff**——同一推理引擎抓不到自己的盲点
...
- `references/rust-anti-patterns.md` — Rust 特定（错误处理 / 边界可见性 / serde / 测试陷阱 / 模块组织；async / cache byte cap / runtime 反模式见 `.claude/rules/perf.md` 真相源）
