...
CLAUDE.md / opsx-apply-cadence.md 等其它文件需要引用触发词或停手词时，**SHALL** 写"见 `.claude/skills/preflight/SKILL.md::Q4`"而**不**各自维护一份分叉副本——本仓库历史上多次踩"同一概念三处独立维护，改一处漏两处"的坑（见 commit b77fdd7 codex 二审）。

- **开工信号词**：`优化 / 改 / 修 / 加 / 做 / 帮我 / 能不能 / 实现 / 重写 / 重构 / polish` 等动词；中英文混用、大小写不敏感（匹配按子串 + 词义）
- **停手词**：见上表第 1-4 行
- 同时含开工词 + 停手词时，**停手词优先**——典型如"帮我看看这段代码 → 仅出方案不动代码"
