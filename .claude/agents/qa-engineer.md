...
1. 读 task 关联的设计意图 + 已有 vitest / Playwright 测试
2. **选验证手段**（不要默认跑 mockIPC 单测就声称端到端通过）：
...
3. 跑 → 判定 → 写报告 → SendMessage lead
4. 发现伪覆盖 SHALL 标注具体测试位置（文件 + 行号）+ 期望行为 + 实际覆盖差距，让 lead 派回前端 / 后端补单测
5. 跨平台 task：本机平台跑完 SHALL flag "Win / Linux 需对应 runner / VM 验证"
